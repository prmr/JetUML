...
public class TestPersistenceService {
	...
	@ParameterizedTest
	@ValueSource(strings = { "testPersistenceService.class.jet", "testPersistenceService2.class.jet",
			"testPersistenceService.sequence.jet", "testPersistenceService.state.jet",
			"testPersistenceService.object.jet", "testPersistenceService.usecase.jet" })
	void test(String pFileName) throws Exception {
		...
		PersistenceTestUtils.getAllNodes(diagram)
				.forEach(node -> bounds.put(hash(diagram, node), renderer.getBounds(node)));
		...
		PersistenceTestUtils.getAllNodes(diagram2).forEach(node -> assertEquals(bounds.get(hash(diagram2, node)),
				renderer2.getBounds(node), hash(diagram2, node)));
		diagram2.edges().forEach(edge -> assertEquals(bounds.get(hash(diagram2, edge)), renderer2.getBounds(edge),
				hash(diagram2, edge)));
	}

	/*
	 * @return A string that is intended to uniquely represent the diagram element
	 * within a diagram, in a way that is resilient to serialization.
	 */
	private static String hash(Diagram pDiagram, DiagramElement pElement) {
		...
		// Call nodes don't have any properties, so we add their order in the control
		// flow
		...
		// Point nodes don't have any properties, so we add the properties of the other
		// node
