...
public class TestEdgeProperties {
	...
	private static List<PropertyName> getPropertyNamesAsList(Edge pEdge) {
		return StreamSupport.stream(pEdge.properties().spliterator(), false).map(Property::name)
				.collect(Collectors.toList());
	}

	private static Stream<Arguments> argumentProvider() {
		return Stream.of(
				Arguments.of(new AggregationEdge(),
						new PropertyName[] { PropertyName.AGGREGATION_TYPE, PropertyName.START_LABEL,
								PropertyName.MIDDLE_LABEL, PropertyName.END_LABEL }),
				Arguments.of(new AssociationEdge(),
						new PropertyName[] { PropertyName.DIRECTIONALITY, PropertyName.START_LABEL,
								PropertyName.MIDDLE_LABEL, PropertyName.END_LABEL }),
				Arguments.of(new CallEdge(), new PropertyName[] { PropertyName.SIGNAL, PropertyName.MIDDLE_LABEL }),
				Arguments.of(new DependencyEdge(),
						new PropertyName[] { PropertyName.MIDDLE_LABEL, PropertyName.DIRECTIONALITY }),
