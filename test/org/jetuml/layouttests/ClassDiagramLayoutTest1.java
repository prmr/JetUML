 ...
 */
public class ClassDiagramLayoutTest1 extends AbstractClassDiagramLayoutTest {

	private static final Path PATH = Path.of("testdata", "testPersistenceService.class.jet");

	public ClassDiagramLayoutTest1() throws IOException {
		super(PATH);
	}

	/*
	 * Tests that nodes are in the position that corresponds to their position value
	 * in the file. We don't test Node7 because its position is calculated from its
	 * children
	 */
	@ParameterizedTest
	@CsvSource({ "Node1, 200, 10", "Node2, 30, 130", "Node3, 200, 130", "Node4, 370, 130", "Node5, 200, 280",
			"Node6, 440, 290" })
	...
	void testImplementationEdge() {
		...
		Rectangle boundsNode3 = aRenderer.getBounds(nodeByName("Node3"));
		GeneralizationEdge edge = (GeneralizationEdge) edgesByType(GeneralizationEdge.class).stream().filter(e -> e
				.properties().get(PropertyName.GENERALIZATION_TYPE).get() == GeneralizationEdge.Type.Implementation)
				.findFirst().get();
	...
	void testInheritanceEdge() {
		...
		GeneralizationEdge edge = (GeneralizationEdge) edgesByType(GeneralizationEdge.class).stream().filter(
				e -> e.properties().get(PropertyName.GENERALIZATION_TYPE).get() == GeneralizationEdge.Type.Inheritance)
				.findFirst().get();
		aRenderer.getBounds(); // Trigger a layout pass
		Rectangle edgeBounds = aRenderer.getBounds(edge);
	...
	void testAggregationEdge() {
		...
		AggregationEdge edge = (AggregationEdge) edgesByType(AggregationEdge.class).stream().filter(
				e -> e.properties().get(PropertyName.AGGREGATION_TYPE).get() == AggregationEdge.Type.Aggregation)
				.findFirst().get();
		aRenderer.getBounds(); // Triggers a layout pass
		Rectangle edgeBounds = aRenderer.getBounds(edge);
	...
	void testCompositionEdge() {
		...
		Rectangle boundsNode4 = aRenderer.getBounds(nodeByName("Node4"));
		AggregationEdge edge = (AggregationEdge) edgesByType(AggregationEdge.class).stream().filter(
				e -> e.properties().get(PropertyName.AGGREGATION_TYPE).get() == AggregationEdge.Type.Composition)
				.findFirst().get();
		aRenderer.getBounds(); // Triggers a layout pass
		Rectangle edgeBounds = aRenderer.getBounds(edge);
	...
	void testNoteEdge() {
		...
		Rectangle boundsNode4 = aRenderer.getBounds(nodeByName("Node4"));
		NoteEdge edge = (NoteEdge) edgesByType(NoteEdge.class).stream().findFirst().get();
		Rectangle edgeBounds = aRenderer.getBounds(edge);
