 ...
 */
public class ObjectDiagramLayoutTest extends AbstractObjectDiagramLayoutTest {

	private static final Path PATH = Path.of("testdata", "testPersistenceService.object.jet");

	ObjectDiagramLayoutTest() throws IOException {
		super(PATH);
	}
	 ...
	 */
	@ParameterizedTest
	@CsvSource({ ":Type1, 240, 130", "object2:, 540, 150", ":Type3, 610, 300", "A note, 280, 330", ":Type4, 440, 290" })
	...
	/**
	 * Tests that the (self) reference edge connects to the node ":Type1" boundary
	 * and falls within the node ":Type1".
	 */
	@Test
	void testSelfReferenceEdge() {
		Node type1Node = nodeByName(":Type1");
		Edge referenceEdge = edgesByType(ObjectReferenceEdge.class).stream()
				.filter(edge -> edge.end().equals(type1Node)).collect(toList()).get(0);
		Rectangle boundsType1Node = aRenderer.getBounds(type1Node);
	...
	/**
	 * Tests that the reference edge connects to the node ":Type4" boundary and
	 * falls within the node ":Type1".
	 */
	@Test
	void testReferenceEdgeBetweenType1NodeAndType4Node() {
		...
		Node type4Node = nodeByName(":Type4");
		Edge referenceEdge = edgesByType(ObjectReferenceEdge.class).stream()
				.filter(edge -> edge.end().equals(type4Node)).collect(toList()).get(0);
		Rectangle boundsType1Node = aRenderer.getBounds(type1Node);
		Rectangle boundsType4Node = aRenderer.getBounds(type4Node);
	...
	/**
	 * Tests that the reference edge connects to the node ":Type3" boundary and
	 * falls within the node ":Type4".
	 */
	@Test
	void testReferenceEdgeBetweenType4NodeAndType3Node() {
		...
		Edge referenceEdge = edgesByType(ObjectReferenceEdge.class).stream()
				.filter(edge -> edge.end().equals(type3Node)).collect(toList()).get(0);
		Rectangle boundsType4Node = aRenderer.getBounds(type4Node);
