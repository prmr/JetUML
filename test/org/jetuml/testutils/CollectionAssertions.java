...
/**
 * A collection of utility methods for testing assertions on collections.
 */
public enum CollectionAssertions {
	...
	/**
	 * True if the elements in the collection are equal to the elements provided as
	 * arguments, in the same sequence
	 */
	...
	/**
	 * True if the set of elements in the collection is exactly the same as the set
	 * of elements provided as arguments (i.e., irrespective of order), using object
	 * equality to test membership.
	 */
	...
	/**
	 * Checks that pAssertion holds for pArguments. If it does, nothing happens If
	 * the assertion does not hold, throws an AssertionError.
	 * 
	 ...
	 */
	public static void assertThat(Collection<?> pCollection, CollectionAssertions pAssertion, Object... pArguments) {
	...
	private static void hasSize(Collection<?> pCollection, Object[] pArguments) {
		...
		if (pCollection.size() != ((Number) pArguments[0]).longValue()) {
			throw new AssertionError(String.format("Collection has size <%d> but expected <%d>", pCollection.size(),
					((Number) pArguments[0]).longValue()));
		}
	}
	...
	private static void hasElementsEqualTo(Collection<?> pCollection, Object[] pArguments) {
		...
		for (int i = 0; i < elements.length; i++) {
			if (!safeEquals(elements[i], pArguments[i])) {
				throw new AssertionError(String.format("Mismatch at position %d: expected %s but got %s ", i,
						pArguments[i], elements[i]));
			}
		}
	}
		...
		for (int i = 0; i < elements.length; i++) {
			if (elements[i] != pArguments[i]) {
				throw new AssertionError(
						String.format("Mismatch at position %d: not the same object as in target sequence", i,
								pArguments[i], elements[i]));
			}
		}
	}
	...
	/**
	 * A convenience method to map the elements in the collection to a value and
	 * create a new collection from the extracted values.
	 * 
	 ...
	 * @param pExtractor  A function to extract the desired elements from the
	 *                    collection elements.
	 * @return A collection of elements mapped from the elements in the input
	 *         collection.
	 */
	...
	private static boolean isInteger(Object pObject) {
		return pObject instanceof Byte || pObject instanceof Short || pObject instanceof Integer
