...
public class EditorFrame extends JFrame
{
	...
	private static final int MARGIN_IMAGE = 2; // Number of pixels to leave around the graph when exporting it as an
												// image
	private static final int HELP_MENU_TEXT_WIDTH = 10; // Number of pixels to give to the width of the text area of the
														// Help Menu.
	private static final int HELP_MENU_TEXT_HEIGHT = 40; // Number of pixels to give to the height of the text area of
															// the Help Menu.
	 ...
	 * 
	 * @param pAppClass
	 *            the fully qualified app class name. It is expected that the resources are appClassName + "Strings" and
	 *            appClassName + "Version" (the latter for version-specific resources)
	 */
	public EditorFrame(Class<?> pAppClass)
	{
		...
		setBounds(screenWidth / (MARGIN_SCREEN * 2), screenHeight / (MARGIN_SCREEN * 2),
				(screenWidth * (MARGIN_SCREEN - 1)) / MARGIN_SCREEN, (screenHeight * (MARGIN_SCREEN - 1)) / MARGIN_SCREEN);
	...
	/**
	 * @param pInternalFrame
	 *            The JInternalFrame to remove. Calling this metod will remove a given JInternalFrame.
	 */
	...
	/*
	 * Rebuilds the "recent files" menu. Only works if the number of recent files is less than 8. Otherwise, additional
	 * logic will need to be added to 0-index the mnemonics for files 1-9
	 */
	...
	public void openFile()
	{
		...
		fileChooser.setFileFilter(new ExtensionFilter(aAppResources.getString("files.name"), aAppResources
				.getString("files.extension")));
		...
		ExtensionFilter[] filters = new ExtensionFilter[] {
				new ExtensionFilter(aAppResources.getString("state.name"), aAppResources.getString("state.extension")
						+ aAppResources.getString("files.extension")),
				new ExtensionFilter(aAppResources.getString("object.name"), aAppResources.getString("object.extension")
						+ aAppResources.getString("files.extension")),
				new ExtensionFilter(aAppResources.getString("class.name"), aAppResources.getString("class.extension")
						+ aAppResources.getString("files.extension")),
	...
	/**
	 * Cuts the current selection of the current panel and puts the content into the application-specific clipboard.
	 */
	public void cut()
	{
		GraphFrame frame = (GraphFrame) aTabbedPane.getSelectedComponent();
	...
	/**
	 * Copies the current selection of the current panel and puts the content into the application-specific clipboard.
	 */
	public void copy()
	{
		GraphFrame frame = (GraphFrame) aTabbedPane.getSelectedComponent();
	...
	/**
	 * Pastes a past selection from the application-specific Clipboard into current panel. All the logic is done in the
	 * application-specific CutPasteBehavior.
	 * 
	 */
	public void paste()
	{
	...
	/**
	 * If a user confirms that they want to close their modified graph, this method will remove it from the current list
	 * of tabs.
	 */
	public void close()
	{
			...
			if (response == JFileChooser.APPROVE_OPTION)
			{
				...
				else
				{
					ResourceBundle editorResources = ResourceBundle
							.getBundle("ca.mcgill.cs.stg.jetuml.framework.EditorStrings");
	...
	/**
	 * Edits the file path so that the pToBeRemoved extension, if found, is replaced with pDesired.
	 * 
	 * @param pOriginal
	 *            the file to use as a starting point
	 * @param pToBeRemoved
	 *            the extension that is to be removed before adding the desired extension.
	 * @param pDesired
	...
	public void exportImage()
	{
		...
		try (OutputStream out = new FileOutputStream(file))
		{
			...
			{
				printSVG(frame.getGraph(), out);
			}
			else if (!ImageIO.getImageWritersByFormatName(format).hasNext())
			{
	...
	private void print(Graphics2D pG2, Graph pGraph)
	{

		Rectangle2D bounds = pGraph.getBounds();
		pG2.translate(-bounds.getX(), -bounds.getY());
		pG2.setColor(Color.WHITE);
		pG2.fill(new Rectangle2D.Double(bounds.getX(), bounds.getY(), bounds.getWidth() + MARGIN_IMAGE * 2, bounds
				.getHeight() + MARGIN_IMAGE * 2));
		pG2.translate(MARGIN_IMAGE, MARGIN_IMAGE);
		pG2.setColor(Color.BLACK);
		pG2.setBackground(Color.WHITE);
		pGraph.draw(pG2, null);
	}

	private void printSVG(Graph pGraph, OutputStream pOut)
	{
		...
		try
		{
			out = new OutputStreamWriter(pOut, "UTF-8");
	...
	private File chooseFileToExportTo()
	{
		...
		FileFilter imageFilter = new FileFilter()
		{
			...
			{
				boolean acceptableFormat = pFile.getName().endsWith(".png") || pFile.getName().endsWith(".jpg") ||
						pFile.getName().endsWith(".svg") || pFile.getName().endsWith(".jpeg"); 
				return !pFile.isDirectory() && acceptableFormat;
			}
		};
	...
	private static BufferedImage getImage(Graph pGraph)
	{
		...
		g2.fill(new Rectangle2D.Double(bounds.getX(), bounds.getY(), bounds.getWidth() + MARGIN_IMAGE * 2, bounds
				.getHeight() + MARGIN_IMAGE * 2));
	...
	public void showAboutDialog()
	{
		...
		JOptionPane
				.showInternalMessageDialog(aTabbedPane, formatter.format(new Object[] {
						aAppResources.getString("app.name"), aVersionResources.getString("version.number"),
						aVersionResources.getString("version.date"), aAppResources.getString("app.copyright"),
	...
	/**
	 * Exits the program if no graphs have been modified or if the user agrees to abandon modified graphs.
	 */
	public void exit()
	{
		int modcount = 0;
		...
		{
			// ask user if it is ok to close
			int result = JOptionPane.showInternalConfirmDialog(aTabbedPane, MessageFormat.format(
					aEditorResources.getString("dialog.exit.ok"), new Object[] { new Integer(modcount) }), null,
					JOptionPane.YES_NO_OPTION);
