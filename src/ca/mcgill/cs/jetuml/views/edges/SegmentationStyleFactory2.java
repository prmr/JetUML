...
import ca.mcgill.cs.jetuml.views.edges.SegmentationStyle2.Side;
...
public final class SegmentationStyleFactory2
{
	 ...
	 */
	public static SegmentationStyle2 createStraightStrategy()
	{
		return new Straight();
	}
	 ...
	 */
	public static SegmentationStyle2 createHVHStrategy()
	{
		return new HVH();
	}
	 ...
	 */
	public static SegmentationStyle2 createVHVStrategy()
	{
		return new VHV();
	}
	...
	private static Point2D findTopRightCorner(Node pNode)
	{
		...
		{
			return ((PackageNodeView2)((PackageNode)pNode).view2()).getTopRightCorner();
		}
		else
		{
			return new Point2D(pNode.view2().getBounds().getMaxX(), pNode.view2().getBounds().getY());
		}
	}
	
	private static class Straight implements SegmentationStyle2
	{
		@Override
	...
	private static boolean hasSelfEdge(Node pNode, Graph2 pGraph)
	{
	 ...
	 */
	private static Position computePosition(Edge pEdge, Side pStartSide, Graph2 pGraph, boolean pForward)
	{
	...
	// TODO Fix this
	private static List<Edge> getAllEdgesForSide(Graph2 pGraph, Node pTarget, Side pSide)
	{
		...
		for( Edge edge : pGraph.getEdges(pTarget))
		{
			...
			if( edge instanceof ClassRelationshipEdge && 
					((ClassRelationshipEdge)edge).obtainSegmentationStyle2().getAttachedSide(edge, pTarget) == pSide )
			{
	...
	private static class HVH implements SegmentationStyle2
	{
		@Override
		public Side getAttachedSide(Edge pEdge, Node pNode)
		{
			...
			{
				SegmentationStyle2 vhv = new VHV();
		...
		public Point2D[] getPath(Edge pEdge, Graph2 pGraph)
		{
			...
			if( !isPossible(pEdge) )
			{
				SegmentationStyle2 alternate = new VHV();
	...
	private static class VHV implements SegmentationStyle2
	{
		@Override
		public Side getAttachedSide(Edge pEdge, Node pNode)
		{
			...
			{
				SegmentationStyle2 hvh = new HVH();
		...
		public Point2D[] getPath(Edge pEdge, Graph2 pGraph)
		{
			...
			if( !isPossible(pEdge) )
			{
				SegmentationStyle2 alternate = new HVH();
				if( alternate.isPossible(pEdge))
				{
					return alternate.getPath(pEdge, pGraph);
				}
