 ...
 */
public class SegmentedEdgeView2 extends AbstractEdgeView2
{
	...
	private SegmentationStyle2 aStyle;
	 ...
	 */
	public SegmentedEdgeView2(Edge pEdge, SegmentationStyle2 pStyle, Supplier<LineStyle> pLineStyle, Supplier<ArrowHead> pStart, 
...
//		pGraphics.draw(getSegmentPath());
	...
	{    
		...
		else 
		{
			...
			if(pArrow != null)
			{
				Bounds arrowBounds = pArrow.view2().getPath(pEndPoint1, pEndPoint2).getBoundsInLocal();
				if(pEndPoint1.getX() < pEndPoint2.getX())
				{
	...
	{
		return aStyle.getPath(edge(), edge().getGraph2());
	}
	...
	{
		Point2D[] points = getPoints();
		return new Line(Conversions2.toPoint(points[0]), 
				Conversions2.toPoint(points[points.length - 1]));
	}
	...
	{		
		...
		Point2D[] points = getPoints();
		path.getElements().addAll(aArrowStartSupplier.get().view2().getPath(points[1], points[0]).getElements());
