 ...
 */
public class ObjectDiagramGraph2 extends Graph2
{
	...
	private ObjectNode findObject(FieldNode pNode, Point pPoint)
	{
		...
		for( Node node : aRootNodes )
		{
			...
			else if( node.view2().contains(pPoint) && canAddNodeAsChild(node, pNode))
			{
	...
	{
		return ResourceBundle.getBundle("ca.mcgill.cs.jetuml.UMLEditorStrings").getString("object2.extension");
	}

	@Override
	public String getDescription() 
	{
		return ResourceBundle.getBundle("ca.mcgill.cs.jetuml.UMLEditorStrings").getString("object2.name");
	}
}
