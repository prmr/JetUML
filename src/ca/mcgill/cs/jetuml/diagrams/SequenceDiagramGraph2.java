 ...
 */
public class SequenceDiagramGraph2 extends Graph2
{
	...
	private void heightObjectLayout(ArrayList<Node> pTopLevelCalls, ArrayList<Node> pObjects)
	{
		...
		for(Node node : aRootNodes )
		{
			if( node instanceof ImplicitParameterNode )
			{
				...
				{
					top = Math.max(top, callNode.view2().getBounds().getY() + callNode.view2().getBounds().getHeight());
				}
			}
		}
		...
		for( Node node : pObjects )
		{
			Rectangle bounds = node.view2().getBounds();
	...
	{
		return ResourceBundle.getBundle("ca.mcgill.cs.jetuml.UMLEditorStrings").getString("sequence2.extension");
	}

	@Override
	public String getDescription() 
	{
		return ResourceBundle.getBundle("ca.mcgill.cs.jetuml.UMLEditorStrings").getString("sequence2.name");
	}
