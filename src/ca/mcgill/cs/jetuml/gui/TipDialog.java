...
public class TipDialog 
{
	...
	private Scene createScene() 
	{
		...
		// Checking that the tipElements are valid, if not switch the tip to the default tip.
		for(TipElement tipElement : tipElements)
		{
			Media tipMedia = tipElement.getMedia();
			if(tipMedia != Media.IMAGE && tipMedia != Media.TEXT)
			{
				tipOfTheDay = TipLoader.getDefaultTip();
				break;
			}
		}
		
		// Adding title to VBox
		Node titleNode = getTipTitleAsNode(tipOfTheDay);
