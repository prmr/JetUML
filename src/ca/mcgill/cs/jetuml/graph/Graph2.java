...
{
	protected GraphModificationListener2 aModificationListener; // Only access from notify* methods and setter
	 ...
	 */
	public void setGraphModificationListener(GraphModificationListener2 pListener)
	{
	...
	public final boolean addEdge(Edge pEdge, Point pPoint1, Point pPoint2)
	{
		...
		pEdge.connect2(node1, node2, this);
	...
	protected Node deepFindNode( Node pNode, Point pPoint )
	{
		...
		if( pNode.view2().contains(pPoint))
		{
			return pNode;
		}
		else
...
//		pGraphics.setBackground(Color.WHITE);
	...
	public Rectangle getBounds()
	{
		...
		for(Node node : aRootNodes )
		{
			if(bounds == null)
			{
				bounds = node.view2().getBounds();
			}
			else
			{
				bounds = bounds.add(node.view2().getBounds());
			}
		}
		for(Edge edge : aEdges)
		{
			bounds = bounds.add(edge.view2().getBounds());
		}
		if(bounds == null )
		{
	...
	{
		pEdge.connect2(pStart, pEnd, this);
		aEdges.add(pEdge);
	}
