...
public final class ClassDiagramRenderer extends AbstractDiagramRenderer {
	...
	/**
	 * Uses positional information of nodes and stored edges to layout and store the
	 * EdgePaths of edges in pDiagram.
	 * 
	...
	/**
	 * Plans the EdgePaths for all segmented edges with EdgePriority pEdgePriority.
	 * 
	...
	private void layoutSegmentedEdges(EdgePriority pEdgePriority) {
		...
		List<Edge> edgesToProcess = diagram().edges().stream().filter(edge -> priorityOf(edge) == pEdgePriority)
				.sorted(Comparator.comparing(edge -> edge.start().position().x())).collect(toList());
		...
		while (!edgesToProcess.isEmpty()) {
			...
			// Determine if currendEdge should merge with other edges at its start node or
			// end node
	...
	private void layoutDependencyEdges() {
		...
		for (Edge edge : diagram().edges()) {
			if (priorityOf(edge) == EdgePriority.DEPENDENCY) { // Determine the start and end connection points
	...
	/**
	 * Gets the node corner where the self-edge pEdge should be placed. If no
	 * corners are available, returns TOP_RIGHT.
	 * 
	 * @param pEdge the self-edge of interest
	 * @return the first available corner on the node, starting with TOP_RIGHT and
	...
	private NodeCorner getSelfEdgeCorner(Edge pEdge) {
		...
		for (NodeCorner corner : NodeCorner.values()) { // Get a 2D array of [startPoint, endPoint] for a self edge at
														// the corner
	 ...
	 * @return the EdgePath for pEdge, beginning on the North or South side of a
	 *         node, ending on the East or West side of the node.
	...
	private EdgePath buildSelfEdge(Edge pEdge, NodeCorner pCorner) {
		...
		Point middleBend;
		Point lastBend;
		// determine location of first bend: either 20px above or 20px below the start
		// point
	...
	/**
	 * Builds and stores the EdgePaths for edges in pEdgesToMergeEnd, so they merge
	 * at a common end point.
	 * 
	 * @param pDirection       the trajectory of the edges in pEdgesToMergeEnd (the
	 *                         direction of the first segment of the edges)
	...
	private void storeMergedEndEdges(Side pDirection, List<Edge> pEdgesToMergeEnd) {
		...
		Point sharedEndPoint = getConnectionPoint(pEdgesToMergeEnd.get(0).end(), pEdgesToMergeEnd.get(0),
				pDirection.mirrored());
		...
		if (pDirection.isHorizontal()) {
			midLineCoordinate = getHorizontalMidLine(closestStartPoint, sharedEndPoint, pDirection,
					pEdgesToMergeEnd.get(0));
		}
		else {
			midLineCoordinate = getVerticalMidLine(closestStartPoint, sharedEndPoint, pDirection,
					pEdgesToMergeEnd.get(0));
		}
		// Build and store each edge's EdgePath
		for (Edge edge : pEdgesToMergeEnd) {
			EdgePath path = buildSegmentedEdgePath(pDirection, startPoints.get(edge), midLineCoordinate,
					sharedEndPoint);
			aEdgeStorage.store(edge, path);
		}
	}
	...
	/**
	 * Builds and stores the EdgePaths for edges in pEdgesToMergeStart so that they
	 * share a common start point.
	 * 
	 * @param pDirection         the trajectory of the edges in pEdgesToMmergeStart
	 *                           (the direction of the first segment of the edges)
	 * @param pEdgesToMergeStart a list of edges which should me merged at their
	 *                           start points
	...
	private void storeMergedStartEdges(Side pDirection, List<Edge> pEdgesToMergeStart) {
		...
		// The default position of the merged middle segment is halfway between
		// startPoint and closestEndPoint:
		...
		if (pDirection.isHorizontal()) {
			...
			midLineCoordinate = getVerticalMidLine(closestEndPoint, startPoint, pDirection, pEdgesToMergeStart.get(0));
		}
		// Build and store each edge's EdgePath
		for (Edge edge : pEdgesToMergeStart) {
	...
	/**
	 * Creates a segmented EdgePath using pStart and pEnd as start and end points
	 * (respectively) and pMidLine as an X or Y coordinate of the middle segment.
	 * 
	 * @param pEdgeDirection the direction describing the trajectory of pEdge
	 * @param pStart         the start point of pEdge
	 * @param pMidLine       an integer representing an X or Y coordinate of the
	 *                       middle segment
	 * @param pEnd           the end point of pEdge
	 * @return a EdgePath consisting of pStart, 2 middle points connecting the
	 *         middle segment, and pEnd
	 ...
	 * @return a list containing the edges which should merge with pEdge (not
	 *         including pEdge itself).
	 * @pre pEdge != null
	 * @pre pEdges != null
	 */
	private List<Edge> getEdgesToMergeEnd(Edge pEdge, List<Edge> pEdges) {
		assert pEdge != null && pEdges != null;
		return pEdges.stream().filter(edge -> edge.end() == pEdge.end())
				...
				.filter(edge -> noOtherEdgesBetween(edge, pEdge, pEdge.end())).filter(edge -> !edge.equals(pEdge))
				.collect(toList());
	}
	...
	private List<Edge> getEdgesToMergeStart(Edge pEdge, List<Edge> pEdges) {
		assert pEdge != null && pEdges != null;
		return pEdges.stream().filter(edge -> edge.start().equals(pEdge.start()))
				.filter(edge -> priorityOf(edge) == priorityOf(pEdge))
				...
				.filter(edge -> noConflictingStartLabels(edge, pEdge)).filter(edge -> !edge.equals(pEdge))
				.collect(toList());
	}
	...
	private int getHorizontalMidLine(Point pStart, Point pEnd, Side pEdgeDirection, Edge pEdge) {
		...
		assert pStart != null && pEnd != null;
		// Check for any edge in storage which is attached to pEdge's start and end
		// nodes
		...
		if (closestStoredEdge.isEmpty()) {
			// If there are no conflicting segments, return the y-coordinate equidistant
			// between the start and end points
			return pEnd.y() + ((pStart.y() - pEnd.y()) / 2);
		}
		else { // Return a y-coordinate either 10px above or 10px below the horizontal middle
				// segment of closestStoredEdge
	 ...
	 * @param pStart         the start point of pEdge
	 * @param pEnd           the end point of pEdge
	 * @param pEdgeDirection the direction of pEdge (the direction of the first
	 *                       segment of pEdge).
	 * @param pEdge          the edge of interest
	 * @return an integer representing the x-coordinate for the vertical middle
	 *         segment of pEdge.
	...
	private int getVerticalMidLine(Point pStart, Point pEnd, Side pEdgeDirection, Edge pEdge) {
		...
		if (closestStoredEdge.isEmpty()) {
			// If no stored edges conflict with pEdge then return the x-coordinate in
			// between the start and end points
			return pEnd.x() + ((pStart.x() - pEnd.x()) / 2);
		}
		else { // Return an x-coordinate either 10px to the left or 10px to the right of
				// closestStoredEdge
	...
	/**
	 * Gets the y-coordinate of the horizontal middle segment of pNewEdge, so that
	 * it avoids overlapping with pEdgeWithSameNodes, which is already in storage.
	 * 
	 * @param pEdgeWithSameNodes an edge in storage which shares the same 2
	 *                           connected nodes as pNewEdge.
	 * @param pNewEdge           the segmented edge whose middle segment we want
	 * @param pEdgeDirection     the direction of pEdge (the direction of the first
	 *                           segment of pEdge).
	 * @return an integer describing the y-coordinate of the middle segment of
	 *         pEdge, so that pEdge and pEdgeWithSameNodes do not overlap.
	...
	/**
	 * Gets the x-coordinate of the vertical middle segment of pNewEdge, so that it
	 * avoids overlapping with pEdgeWithSameNodes, which is already in storage.
	 * 
	 * @param pEdgeWithSameNodes an edge in storage which shares the same 2
	 *                           connected nodes as pNewEdge.
	 ...
	 * @param pEdgeDirection     the direction of pEdge (the direction of the first
	 *                           segment of pEdge).
	 * @return The x-coordinate of the middle segment of pEdge, so that pEdge and
	 *         pEdgeWithSameNodes do not overlap.
	 * @pre pEdgeWithSameNodes is present in storage
	 * @pre pEdgeWithSameNodes and pEdge both share the same 2 attached nodes.
	...
	/**
	 * Gets the closest Edge to pEdge.getEnd() (or pEdge.getStart() for
	 * AggregationEdges) which might interfere with the position of the horizontal
	 * middle segment of pEdge. Considers all stored, segmented edges which are
	 * attached to either of pEdge's nodes and could conflict with pEdge.
	 * 
	 * @param pEdgeDirection the direction describing the trajectory of pEdge (the
	 *                       direction of the first segment of pEdge)
	 * @param pEdge          the edge of interest
	 * @return the closest Edge conflicting with the horizontal middle segment of
	 *         pEdge, or Optional.empty() if there are no conflicting edges.
	 * @pre pEdgeDirection == Direction.NORTH || pEdgeDirection == Direction.SOUTH
	 */
	private Optional<Edge> closestConflictingHorizontalSegment(Side pEdgeDirection, Edge pEdge) {
		...
		// also consider edges which are connected to pEdge.getStart() which are in the
		// way of pEdge
		conflictingEdges.addAll(storedConflictingEdges(pEdgeDirection, pEdge.start(), pEdge));
		if (conflictingEdges.isEmpty()) {
			return Optional.empty();
		}
		else { // For Aggregation/Composition edges: return the Edge with the middle segment
				// which is closest to pEdge's start node
			...
			else { // For all other segmented edges: return the Edge with the middle segment which
					// is closest to pEdge's end node
				return conflictingEdges.stream()
	...
	/**
	 * Gets the closest Edge to pEdge.getEnd() (pEdge.getStart() for
	 * AggregationEdges) which might interfere with the position of the vertical
	 * middle segment of pEdge. Considers all segmented edges which are attached to
	 * both pEdge.getStart() and pEdge.getEnd(), and could conflict with pEdge.
	 * 
	 * @param pEdgeDirection the direction describing the trajectory of pEdge (the
	 *                       direction of the first segment of pEdge)
	 * @param pEdge          the edge of interest
	 * @return the closest Edge conflicting with the vertical middle segment of
	 *         pEdge, or Optional.empty() if there are no conflicting edges.
	 * @pre pEdgeDirection == Direction.EAST || pEdgeDirection == Direction.WEST
	 * @pre EdgePriority.isSegmented(pEdge)
	 */
	private Optional<Edge> closestConflictingVerticalSegment(Side pEdgeDirection, Edge pEdge) {
		assert pEdgeDirection.isVertical();
		assert EdgePriority.isSegmented(pEdge);
		// Get all edges connected to pEdge's end node which could conflict with pEdge's
		// middle segment position
		List<Edge> conflictingEdges = storedConflictingEdges(pEdgeDirection.mirrored(), pEdge.end(), pEdge);
		// Also consider edges connected to pEdge's start node which could conflict with
		// pEdge's middle segment
		conflictingEdges.addAll(storedConflictingEdges(pEdgeDirection, pEdge.start(), pEdge));
		if (conflictingEdges.isEmpty()) {
			return Optional.empty();
		}
		else { // for AggregationEdges: return the Edge with the middle segment which is
				// closest to pEdge.getStart()
			if (pEdge instanceof AggregationEdge) {
				return conflictingEdges.stream().min(
						Comparator.comparing(edge -> horizontalDistanceToNode(pEdge.start(), edge, pEdgeDirection)));
			}
			else { // For all other edges: return the Edge with the middle segment which is closest
					// to pEdge.getEnd()
				return conflictingEdges.stream()
	...
	/**
	 * Gets the y-coordinate for the horizontal middle segment of pEdge based on the
	 * position of pClosestStoredEdge.
	 * 
	 * @param pClosestStoredEdge the closest edge which conflicts with pEdge.
	 * @param pEdge              the edge of interest
	 * @param pEdgeDirection     the direction describing the trajectory of pEdge
	 *                           (the direction of the first segment of pEdge)
	 * @return the y-coordinate for the middle segment of pEdge: either 10px below
	 *         pClosestStoredEdge or 10px above pClosestStoredEdge
	...
	/**
	 * Gets the x-coordinate for the vertical middle segment of pEdge based on the
	 * position of pClosestStoredEdge.
	 * 
	 * @param pClosestStoredEdge the closest edge to pNode which conflicts with
	 *                           pEdge.
	 * @param pEdge              the edge of interest
	 * @param pEdgeDirection     the direction describing the trajectory of pEdge
	 *                           (the direction of the first segment of pEdge).
	 * @return the x-coordinate for the middle segment of pEdge, so that it is
	 *         either 10px to the right of pClosestStoredEdge, or 10px to the left.
	...
	/**
	 * Gets all edges currently in storage which are connected to the pNodeFace side
	 * of pNode with the same index sign (-1 or +1) as pEdge. These edges may
	 * conflict with the default placement of pEdge, so their EdgePaths need to be
	 * considered when planning the EdgePath of pEdge.
	 * 
	 * @param pNodeSide the side of pNode on which pEdge is attached
	 * @param pNode     the node of interest which pEdge is attached to
	 * @param pEdge     the edge of interest
	 * @return a list of edges which could conflict with the default EdgePath of
	 *         pEdge.
	...
	private List<Edge> storedConflictingEdges(Side pNodeSide, Node pNode, Edge pEdge) {
		...
		return aEdgeStorage.edgesConnectedTo(pNode).stream()
				...
				.filter(edge -> !edge.equals(pEdge)).collect(toList());
	}

	/**
	 * Uses EdgeStorage to get the side describing the face of pNode on which a
	 * stored edge pEdge is attached.
	 * 
	 * @param pEdge the edge of interest
	 * @param pNode the node of interest which PEdge is attached to
	...
	private Side attachedSideFromStorage(Edge pEdge, Node pNode) {
		...
		// Iterate over each side of pNode. Return the side which contains
		// connectionPoint
	...
	/**
	 * Gets the vertical distance in pixels between the North side of pEndNode and
	 * the horizontal middle segment of pEdge.
	 * 
	 * @param pEndNode       the end node of interest
	 * @param pEdge          the segmented edge of interest
	 * @param pEdgeDirection the trajectory of pEdge
	 * @return the absolute value of the distance between the North side of pEndNode
	 *         and the middle segment of pEdge in pixels.
	 * @pre pEdgeDirection == Direction.NORTH || pEdgeDirection == Direction.SOUTH
	...
	/**
	 * Gets the horizontal distance between the West side of pNode and the vertical
	 * middle segment of pEdge.
	 * 
	 * @param pEndNode       the node of interest. The end node for pEdge.
	 * @param pEdge          the segmented edge of interest.
	 * @param pEdgeDirection the trajectory of pEdge
	 * @return the absolute value of the distance between the West side of pNode and
	 *         the middle segment of pEdge
	 * @pre pEdgeDirection == Direction.EAST || PEdgeDirection == Direction.WEST
	 * @pre EdgePriority.isSegmented(priorityOf(pEdge));
	 */
	...
	/**
	 * Gets the point from pPoints which is farthest in the direction
	 * pEdgeDirection. For example, if pDirection is North, it will return the
	 * Northern-most point from pPoints.
	 * 
	...
	private static Point getClosestPoint(Collection<Point> pPoints, Side pDirection) {
		...
		if (pDirection == Side.TOP) {// Then return the point with the smallest Y-coordinate
			return pPoints.stream().min((p1, p2) -> Integer.compare(p1.y(), p2.y())).orElseGet(null);
		}
		else if (pDirection == Side.BOTTOM) {// Then return the point with the the largest Y-coordinate
			return pPoints.stream().max((p1, p2) -> Integer.compare(p1.y(), p2.y())).orElseGet(null);
		}
		else if (pDirection == Side.RIGHT) {// Then return the point with the largest X-coordinate
			return pPoints.stream().max((p1, p2) -> Integer.compare(p1.x(), p2.x())).orElseGet(null);
		}
		else { // Then return the point with the smallest X-coordinate
			return pPoints.stream().min((p1, p2) -> Integer.compare(p1.x(), p2.x())).orElseGet(null);
		}
	}

	/**
	 * Checks whether the start labels of pEdge1 and pEdge2 are equal, if they both
	 * have start labels.
	 * 
	 * @param pEdge1 an edge of interest
	 * @param pEdge2 another edge of interest
	 * @return false if the edges are both ThreeLabelEdge edges with different start
	 *         labels. True otherwise.
	...
	/**
	 * Checks whether the end labels of pEdge1 and pEdge2 are equal, if they both
	 * have end labels.
	 * 
	 * @param pEdge1 an edge of interest
	 * @param pEdge2 another edge of interest
	 * @return false if the edges are both ThreeLabelEdge edges with different end
	 *         labels. True otherwise.
	 * @pre pEdge1 !=null && pEdge2 !=null
	 */
	...
	/**
	 * Returns whether there are any edges connected to pNode in between pEdge1 and
	 * pEdge2.
	 * 
	 * @param pEdge1 an edge of interest
	 * @param pEdge2 another edge of interest
	 * @param pNode  the node on which pEdge1 and pEdge2 are attached
	 * @return true if there are no stored edges on pNode which are attached in
	 *         between pEdge1 and pEdge2, false otherwise
	...
	private boolean noOtherEdgesBetween(Edge pEdge1, Edge pEdge2, Node pNode) {
		...
		// Return true if there are no other stored edges connected to the same side of
		// pNode as pEdge1 and pEdge2
		if (aEdgeStorage.edgesConnectedTo(pNode).stream()
				.filter(edge -> attachedSide(edge, pNode) == attachedSide(pEdge1, pNode)).collect(toList()).isEmpty()) {
			return true;
		}
		else { // Compare the center point of pNode to the center points of the other nodes of
				// pEdge1 and pEdge2:
			return nodesOnSameSideOfCommonNode(getOtherNode(pEdge1, pNode), getOtherNode(pEdge2, pNode), pNode,
					attachedSide(pEdge1, pNode));
		}
	}

	/**
	 * Returns whether the center points of pNode1 and pNode2 are both on the same
	 * side relative to the center point of pCommonNode.
	 * 
	 * @param pNode1        a node in the diagram
	 * @param pNode2        another node in the diagram
	 * @param pCommonNode   aNode which has edges which connect to both pNode1 and
	 *                      pNode2
	 * @param pAttachedSide the side of pCommonNode on which the edges from pNode1
	 *                      and pNode2 connect
	 * @return true if the center points of pNode1 and pNode2 are both above, below,
	 *         to the left, or to the right of the center point of pCommonNode.
	 *         False otherwise.
	...
	/**
	 * Gets the connection point for pEdge on the pAttachmentSide of pNode. Checks
	 * with the diagram's EdgeStorage to find the closest available connection point
	 * to the center point (NodeIndex ZERO).
	 * 
	...
	private Point getConnectionPoint(Node pNode, Edge pEdge, Side pAttachmentSide) {
		...
		// Get the first available connection point, starting at NodeIndex ZERO and
		// moving outwards
		...
		// If no connection point was available, return the point at NodeIndex
		// MINUS_FOUR or PLUS_FOUR
	 ...
	 * 
	 * Edges which share both their start and end nodes with another edge
	 * (shared-node edges) should have the same index sign, regardless of whether
	 * they are outgoing or incoming on pNode.
	 * 
	 * For all other edges, the index sign can be obtained by comparing the relative
	 * positions of pNode and the other Node attached to pEdge.
	 * 
	 * @param pEdge       the edge of interest
	 * @param pNode       the node for which we want the index sign
	 * @param pSideOfNode the cardinal direction describing the side of pNode where
	 *                    pEdge attaches
	 * @return the index sign (-1 or 1) describing the attachment of pEdge onto
	 *         pNode.
	 * @pre pEdge.getStart() == pNode || pEdge.getEnd() == pNode
	 */
	private int getIndexSign(Edge pEdge, Node pNode, Side pSideOfNode) {
		assert pEdge.start() == pNode || pEdge.end() == pNode;
		// Check whether there are any stored edges which are connected to both
		// pEdge.getStart() and pEdge.getEnd()
		List<Edge> edgesWithSameNodes = aEdgeStorage.getEdgesWithSameNodes(pEdge);
		if (!edgesWithSameNodes.isEmpty()) { // For shared-nod edge: index sign on start node should always be same as
												// end node index sign
	...
	/**
	 * Gets the index sign (-1 or 1) describing the attachment of pNode on pEdge.
	 * Compares the relative positions of pNode and pOtherNode to determine if the
	 * index sign should be -1 or +1.
	 * 
	 * @param pEdge       the edge of interest
	...
	private Side attachedSide(Edge pEdge, Node pNode) {
		...
		/*
		 * If there is an Edge in storage which is also attached to pEdge.getStart() and
		 * pEdge.getEnd() then the attachment side of pEdge onto pNode must be the same
		 * as the attachment side of the stored node onto pNode. (These are referred to
		 * as "shared-node edges").
		 */
		List<Edge> edgesWithSameNodes = aEdgeStorage.getEdgesWithSameNodes(pEdge);
		if (!edgesWithSameNodes.isEmpty()) {
			return attachedSideFromStorage(edgesWithSameNodes.get(0), pNode);
		}
		// AggregationEdges prefer to attach on East/West sides, unless nodes are
		// directly above/below each other
		if (pEdge instanceof AggregationEdge) {
			startAttachedSide = attachedSidePreferringEastWest(pEdge);
		}
		// Other edges prefer to attach on North/South sides, unless nodes are directly
		// beside each other
		else {
			startAttachedSide = attachedSidePreferringNorthSouth(pEdge);
		}
		// The attached side of pEdge to its end node is always opposite of the
		// attachment side to its start node
		if (isOutgoingEdge(pEdge, pNode)) {
	...
	/**
	 * Gets the cardinal direction describing the side of pEdge's start node on
	 * which pEdge should attach. pEdge should connect to the East or West sides
	 * unless its nodes are directly above or below each other.
	 * 
	 * @param pEdge the edge of interest
	...
	private Side attachedSidePreferringEastWest(Edge pEdge) {
		...
		// if the start node is above or below the end node (+- 20 px) then determine
		// whether it belongs on the N or S side
	 ...
	 * 
	 * @param pEdge the edge of interest
	 * @return the North/South side of pEdge's start node which pEdge should attach
	 *         to, unless pEdge's nodes are directly beside each other.
	 */
	private Side attachedSidePreferringNorthSouth(Edge pEdge) {
		assert pEdge != null;
		Rectangle startNodeBounds = getBounds(pEdge.start());
		Rectangle endNodeBounds = getBounds(pEdge.end());
		// if the start node is beside the end node (+- 20 px) then compute whether it
		// belongs on the E or W side
	...
	/**
	 * Returns the side of pEdge's start node that pEdge should attach to. This side
	 * will be either North or South, unless pEdge's nodes are closer to each other
	 * than they are to pEdge's middle segment. In that case, it returns the
	 * East/West attachment side.
	 * 
	 * @param pEdge the edge of interest
	 * @return the attachment side of pEdge
	 */
	private Side northSouthSideUnlessTooClose(Edge pEdge) {
		Side preferredSide = northOrSouthSide(getBounds(pEdge.start()), getBounds(pEdge.end()));
	...
	/**
	 * Returns whether pEdge should attach on the East or West side of its start
	 * node, unless pEdge's nodes are too close together, in which case it returns
	 * the North/South attachment side.
	 * 
	 * @param pEdge the edge of interest
	 * @return the attachment side of pEdge
	 */
	private Side eastWestSideUnlessTooClose(Edge pEdge) {
		Side preferredSide = eastOrWestSide(getBounds(pEdge.start()), getBounds(pEdge.end()));
		if (nodeIsCloserThanSegment(pEdge, pEdge.end(), preferredSide) ||
				nodeIsCloserThanSegment(pEdge, pEdge.start(), preferredSide.mirrored())) {
	...
	/**
	 * Compares the relative positions of pBounds and pOtherBounds to determine if
	 * an edge connecting pBounds and pOtherBounds should connect on the North or
	 * South side of pBounds.
	 * 
	 * @param pBounds      the rectangle of interest
	 * @param pOtherBounds another rectangle to compare with
	 * @return the side of pBounds (either North or South) where an edge connecting
	 *         pBounds and pOtherBounds should attach.
	...
	/**
	 * Compares the relative positions of pBounds and pOtherBounds to determine if
	 * an edge connecting pBounds and pOtherBounds should connect on the East or
	 * West side of pBounds.
	 * 
	 * @param pBounds      the rectangle of interest
	 * @param pOtherBounds another rectangle to compare with
	 * @return the side of pBounds (either East or West) where an edge connecting
	 *         pBounds and pOtherBounds should attach.
	 * @pre pBounds !=null && pOtherBounds !=null
	 */
	...
	/**
	 * Returns whether the other node attached to pEdge is closer to pNode than
	 * pEdge's middle segment.
	 * 
	 * @param pEdge         the edge of interest
	 * @param pNode         a node attached to pEdge
	 * @param pAttachedSide the side of pNode where pEdge is attached
	 * @return true if pEdge's other node is closer to pNode than pEdge's middle
	 *         segment is, false otherwise.
	 * @pre pEdge.getStart() == pNode || pEdge.getEnd() == pNode
	 */
	private boolean nodeIsCloserThanSegment(Edge pEdge, Node pNode, Side pAttachedSide) {
		...
		if (pAttachedSide == Side.TOP) { // Consider the middle segments of edges attached to pNode
		...
		else if (pAttachedSide == Side.BOTTOM) {// Consider the middle segments of edges attached to pNode
			return !storedConflictingEdges(pAttachedSide.mirrored(), pNode, pEdge).stream()
					.filter(edge -> otherNodeBounds.maxY() > getEdgePath(edge).getPointByIndex(1).y() + TEN_PIXELS)
					.collect(toList()).isEmpty();
		}
		else if (pAttachedSide == Side.RIGHT) {// Consider the middle segments of edges attached to pNode
			return !storedConflictingEdges(pAttachedSide.mirrored(), pNode, pEdge).stream()
	...
	/**
	 * Returns an array of [startPoint, endPoint] where a self-edge attached to the
	 * pCorner of pNode would connect.
	 * 
	 * @param pCorner the NodeCorner of interest
	 * @param pNode   the node of interest
	 * @return an array containing the start point and end point for a self edge
	 *         attached to the pCorner corner of pNode.
	 */
