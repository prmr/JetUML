 ...
 * types.
 * 
 * A rendering pass starts with a call to draw(...). This results in the
 * complete geometry of the diagram being computed, and some of the computations
 * being cached. Subsequent calls to query method will use the cached
 * computations.
 * 
 * A single instance of each specialized renderer is needed as long as the
 * geometry is recomputed with a call to draw before any querying of the diagram
 * geometry.
 */
public interface DiagramRenderer {
	/**
	 * Computes the geometry of the diagram and draws the diagram onto the graphics
	 * context.
	 * 
	 ...
	 * @return An edge containing pPoint or Optional.empty() if no edge is under
	 *         pPoint
	...
	/**
	 * Returns the node underneath the given point, if it exists. Always returns the
	 * deepest child and the last one in a list.
	 * 
	 * @param pPoint A point
	 * @return a node containing pPoint or Optional.empty() if no node is under
	 *         pPoint
	 * @pre pPoint != null.
	 */
	 ...
	 * selected. The difference between this method and nodeAt is that it is
	 * specialized for nodes that can be selected by the users, whereas nodeAt is
	 * also used for edge creation. By default, this method has the same behavior as
	 * nodeAt.
	 * 
	 * @param pPoint A point
	 ...
	 * @return The diagram element renderer for a given diagram element class, that
	 *         is managed by this renderer.
	 */
	 ...
	 * @return A rectangle that represents the bounding box of the entire selection
	 *         including the bounds of their parent nodes.
	 * @pre pElements != null
	 ...
	 * @param pElements The elements whose bounds we are interested in.
	 * @return A rectangle that represents the bounding box of the entire selection
	 *         but exclusing the bounds of their parent nodes.
	 * @pre pElements != null
