...
public abstract class AbstractPackageNodeRenderer extends AbstractNodeRenderer {
	...
	public void draw(DiagramElement pElement, RenderingContext pContext) {
		...
		pContext.drawRectangle(topBounds, ColorScheme.get().fill(), ColorScheme.get().stroke(),
				Optional.of(ColorScheme.get().dropShadow()));
		pContext.drawRectangle(bottomBounds, ColorScheme.get().fill(), ColorScheme.get().stroke(),
				Optional.of(ColorScheme.get().dropShadow()));
		LABEL_RENDERER.draw(((AbstractPackageNode) pElement).getName(),
				new Rectangle(topBounds.x() + HORIZONTAL_PADDING, topBounds.y(), topBounds.width(), topBounds.height()),
				pContext);
	}
	...
	public Point getConnectionPoint(Node pNode, Direction pDirection) {
		...
		if (connectionPoint.y() < bottomBounds.y() && topBounds.maxX() < connectionPoint.x()) {
			...
			int delta = topBounds.height() * (connectionPoint.x() - bounds.center().x()) * 2 / bounds.height();
	...
	/*
	 * The top face of a package node is only the side of the bottom (main)
	 * rectangle.
	 */
	@Override
	 ...
	 * @return The point that corresponds to the actual top right corner of the
	 *         figure (as opposed to bounds).
	 */
