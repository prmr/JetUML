...
public final class NoteNodeRenderer extends AbstractNodeRenderer {
	...
	public void draw(DiagramElement pElement, RenderingContext pContext) {
		...
		NOTE_VIEWER.draw(((NoteNode) node).getName(),
				new Rectangle(node.position().x() + PADDING, node.position().y() + PADDING,
						DEFAULT_WIDTH - FOLD_LENGTH - PADDING * 2, DEFAULT_HEIGHT - FOLD_LENGTH - PADDING - 2),
				pContext);
	}
	...
	private Path createNotePath(Node pNode) {
		...
		path.getElements().addAll(new MoveTo(bounds.x(), bounds.y()),
				new LineTo(bounds.maxX() - FOLD_LENGTH, bounds.y()),
				new LineTo(bounds.maxX(), bounds.y() + FOLD_LENGTH), new LineTo(bounds.maxX(), bounds.maxY()),
				new LineTo(bounds.x(), bounds.maxY()), new LineTo(bounds.x(), bounds.y()));
		return path;
	}
	...
	private Path createFoldPath(Node pNode) {
		Rectangle bounds = getBounds(pNode);
		Path path = new Path();
		path.getElements().addAll(new MoveTo(bounds.maxX() - FOLD_LENGTH, bounds.y()),
