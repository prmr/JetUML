 ...
 * 
 * The top box, which shows the title, has a minimum height of 20 pixels, minus
 * 20 if attributes are present, minus another 20 if methods are present.
 */
public class TypeNodeRenderer extends AbstractNodeRenderer {
	...
	private static final StringRenderer ITALIC_NAME_RENDERER = new StringRenderer(Alignment.CENTER, Decoration.BOLD,
			Decoration.ITALIC);
	private static final StringRenderer STRING_RENDERER = new StringRenderer(Alignment.LEFT);
	private static final StringRenderer UNDERLINING_STRING_RENDERER = new StringRenderer(Alignment.LEFT,
			Decoration.UNDERLINED);
	private static final StringRenderer ITALIC_STRING_RENDERER = new StringRenderer(Alignment.LEFT, Decoration.ITALIC);
	...
	public void draw(DiagramElement pElement, RenderingContext pContext) {
		...
		if (attributeBoxHeight > 0) {
			...
			pContext.strokeLine(bounds.x(), splitY, bounds.maxX(), splitY, ColorScheme.get().stroke(), LineStyle.SOLID);
			drawAttribute(node.getAttributes(), new Rectangle(bounds.x(), splitY, bounds.width(), attributeBoxHeight),
					pContext);
			if (methodBoxHeight > 0) {
				final int splitY2 = splitY + attributeBoxHeight;
				pContext.strokeLine(bounds.x(), splitY2, bounds.maxX(), splitY2, ColorScheme.get().stroke(),
						LineStyle.SOLID);
				drawMethod(node.getMethods(), new Rectangle(bounds.x(), splitY2, bounds.width(), methodBoxHeight),
						pContext);
			}
		}
		...
		for (String attribute : attributesByLine) {
			if (containsMarkup(attribute, UNDERLINE_MARKUP)) {
				UNDERLINING_STRING_RENDERER.draw(removeMarkup(attribute),
						new Rectangle(pBounds.x() + HORIZONTAL_PADDING, pBounds.y() + lineSpacing, pBounds.width(),
								lineHeight()),
						pContext);
			}
			else {
				STRING_RENDERER.draw(attribute, new Rectangle(pBounds.x() + HORIZONTAL_PADDING,
						pBounds.y() + lineSpacing, pBounds.width(), lineHeight()), pContext);
			}
	...
	private static void drawMethod(String pText, Rectangle pBounds, RenderingContext pContext) {
		...
		for (String method : methodsByLine) {
			if (containsMarkup(method, UNDERLINE_MARKUP)) {
				UNDERLINING_STRING_RENDERER.draw(removeMarkup(method), new Rectangle(pBounds.x() + HORIZONTAL_PADDING,
						pBounds.y() + lineSpacing, pBounds.width(), lineHeight()), pContext);
			}
			else if (containsMarkup(method, ITALIC_MARKUP)) {
				ITALIC_STRING_RENDERER.draw(removeMarkup(method), new Rectangle(pBounds.x() + HORIZONTAL_PADDING,
						pBounds.y() + lineSpacing, pBounds.width(), lineHeight()), pContext);
			}
			else {
				STRING_RENDERER.draw(method, new Rectangle(pBounds.x() + HORIZONTAL_PADDING, pBounds.y() + lineSpacing,
						pBounds.width(), lineHeight()), pContext);
			}
			lineSpacing += lineHeight();
		}
	}
	...
	private static Dimension textDimensions(String pString) {
		...
		if (pString.length() > 0) {
			...
			result = new Dimension(dimension.width() + 2 * HORIZONTAL_PADDING,
					dimension.height() + 2 * VERTICAL_PADDING);
		}
		return result;
	}
