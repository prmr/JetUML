...
public final class FieldNodeRenderer extends AbstractNodeRenderer {
	...
	public void draw(DiagramElement pElement, RenderingContext pContext) {
		...
		EQUALS_VIEWER.draw(EQUALS, new Rectangle(split - midOffset, bounds.y(), midOffset * 2, bounds.height()),
				pContext);
		VALUE_VIEWER.draw(((FieldNode) node).getValue(),
				new Rectangle(split + midOffset, bounds.y(), rightWidth(node), bounds.height()), pContext);
	}
	...
	public static int getHeight(Node pNode) {
		assert FieldNode.class.isInstance(pNode);
		return Math.max(DEFAULT_HEIGHT,
				Math.max(NAME_VIEWER.getDimension(((FieldNode) pNode).getName()).height(),
