...
public final class UseCaseNodeRenderer extends AbstractNodeRenderer {
	...
	public void draw(DiagramElement pElement, RenderingContext pContext) {
		...
		pContext.drawOval(bounds.x(), bounds.y(), bounds.width(), bounds.height(), ColorScheme.get().fill(),
				ColorScheme.get().stroke(), Optional.of(ColorScheme.get().dropShadow()));
		String name = ((UseCaseNode) pElement).getName();
		LABEL_RENDERER.draw(name, getBounds(pElement).centerSlice(LABEL_RENDERER.getDimension(name).height()),
				pContext);
	}
	...
	protected Rectangle internalGetBounds(Node pNode) {
		return new Rectangle(pNode.position().x(), pNode.position().y(),
				Math.max(DEFAULT_WIDTH,
						LABEL_RENDERER.getDimension(((UseCaseNode) pNode).getName()).width() + HORIZONTAL_NAME_PADDING),
