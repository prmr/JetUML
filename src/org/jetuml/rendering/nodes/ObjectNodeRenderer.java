...
public final class ObjectNodeRenderer extends AbstractNodeRenderer {
	...
	private static final StringRenderer LABEL_RENDERER = new StringRenderer(Alignment.CENTER, Decoration.BOLD,
	...
	public void draw(DiagramElement pElement, RenderingContext pContext) {
		...
		if (((ObjectNode) node).getChildren().size() > 0) {
			pContext.strokeLine(bounds.x(), dividerPosition, bounds.maxX(), dividerPosition, ColorScheme.get().stroke(),
					LineStyle.SOLID);
		}
		Rectangle top = new Rectangle(bounds.x(), bounds.y(), bounds.width(), topRectangle.height());
		LABEL_RENDERER.draw(((ObjectNode) node).getName(), top.centerSlice(LABEL_RENDERER.lineHeight()), pContext);
	}
	 ...
	 * @return The position that represents the split between the name and value
	 *         fields.
	 */
