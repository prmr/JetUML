...
/**
 * The renderer for sequence diagrams. The implementation of this renderer
 * assumes that any sequence diagram has either zero call nodes or a single root
 * call node, namely, only one call node with no callers.
 */
public final class SequenceDiagramRenderer extends AbstractDiagramRenderer {
	...
	/*
	 * Initial position of the lifeline of an object if it is not the target of a
	 * constructor call.
	 */
	private static final int INITIAL_Y_POSITION = 80;

	/*
	 * Minimum number of pixels to drop a new call edge from the current position in
	 * the call sequence. Should then be adjusted based on font size. See method
	 * getDropDistance()
	 */
	private static final int DROP_MIN = 20;

	/*
	 * Number of pixels to drop a constructor call node from the current position in
	 * the call sequence. Independent of font size.
	 */
	...
	/*
	 * Computes the position of the nodes in the sequence diagram, except the note
	 * nodes.
	 */
	private void layout() {
		computeYPositions();
	 ...
	 *         diagram is empty, but also because it has been loaded from disk and
	 *         before a rendering pass has been done.
	 */
	...
	public boolean topRectangleContains(Node pNode, Point pPoint) {
		...
		assert pPoint != null;
		return ((ImplicitParameterNodeRenderer) rendererFor(pNode.getClass())).getTopRectangle(pNode).contains(pPoint);
	}
	 ...
	 * @return The CallNode that has a outgoing edge terminated at pNode, if there
	 *         is one.
	 * @pre pNode != null && contains(pNode)
	 */
	public Optional<CallNode> getCaller(Node pNode) {
		assert pNode != null && diagram().contains(pNode);
		return diagram().edges().stream().filter(CallEdge.class::isInstance).filter(edge -> edge.end() == pNode)
				.map(Edge::start).map(CallNode.class::cast).findFirst();
	}

	/**
	 * @return The number of call nodes active on the same parent when this call
	 *         nodes gets executed. An initial (non-nested) call node on an implicit
	 *         parameter get result 0. Nesting is generated not only by self-calls,
	 *         but also by any call back to the parent object.
	 */
	...
	/*
	 * Computes the y position of the pNode call node, and all its callees, through
	 * recursive descent. Also adjust the parent in case it's a constructor call.
	 */
	...
	private boolean isConstructorCall(Node pNode) {
		...
		return getIncomingCall(pNode).filter(edge -> edge instanceof ConstructorEdge) // Exists and is a constructor edge
				.isPresent();
	}

	private Optional<Edge> getIncomingCall(Node pNode) {
		assert pNode.getClass() == CallNode.class;
		return diagram().edges().stream().filter(edge -> edge.end() == pNode).findFirst();
	}
	...
	public int getLifelineTop(ImplicitParameterNode pNode) {
		...
		Optional<Node> topNode = diagram().allNodes().stream().filter(CallNode.class::isInstance)
				.filter(node -> node.getParent() == pNode).sorted(Comparator.comparing(aCallNodeTopCoordinate::get))
				.findFirst();
	...
	private Optional<Node> findRoot() {
		Set<Node> calledNodes = diagram().edges().stream()
				...
				.map(Edge::end).collect(Collectors.toSet());
		List<Node> rootNode = diagram().allNodes().stream().filter(node -> node.getClass() == CallNode.class)
				.filter(node -> !calledNodes.contains(node)).collect(Collectors.toList());
	...
	protected Optional<Node> deepFindNode(Node pNode, Point pPoint) {
		...
		if (pNode.getClass() == CallNode.class) {
			result = getCallees(pNode).stream().map(node -> deepFindNode(node, pPoint)).filter(Optional::isPresent)
	...
	/**
	 * Returns the list of nodes directly called by pNode, in the order of the call
	 * sequence.
	 * 
	 * @param pNode The node to obtain the callees for.
	 * @return All Nodes pointed to by an outgoing edge starting at pNode, or the
	 *         empty list if there are none.
	 * @pre pNode != null && contains(pNode)
	 */
	private List<Node> getCallees(Node pNode) {
		assert pNode != null && diagram().contains(pNode);
		return diagram().edges().stream().filter(CallEdge.class::isInstance).filter(edge -> edge.start() == pNode)
				.map(Edge::end).collect(toList());
	}
	...
	/*
	 * @return The number of pixels to drop the call edges from the current position
	 * in the call sequence. Takes into account the size of the font to ensure
	 * labels on call edges do not overlap.
	 */
	...
	/**
	 * Returns the start node for pEdge if it is exclusive to pEdge, namely if it
	 * only has pEdge as an outgoing call edge.
	 * 
	...
	public Optional<DiagramElement> getStartNodeIfExclusive(Edge pEdge) {
		...
		if (startNodeOnlyCalls(pEdge)) {
			...
			// We delete the start node of pEdge if it does not have any caller and only
			// makes calls to the
	...
	private boolean startNodeOnlyCalls(Edge pEdge) {
		...
		return getCaller(pEdge.start()).isEmpty() && calls.size() == 1 && calls.contains(pEdge);
	}
	...
	private List<CallEdge> getCalls(Node pCaller) {
		assert pCaller != null;
		return diagram().edges().stream().filter(CallEdge.class::isInstance).map(CallEdge.class::cast)
				.filter(edge -> edge.start() == pCaller).collect(toList());
	}
}
