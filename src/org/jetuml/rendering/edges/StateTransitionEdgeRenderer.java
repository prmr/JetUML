...
public final class StateTransitionEdgeRenderer extends AbstractEdgeRenderer {
	...
	public void draw(DiagramElement pElement, RenderingContext pContext) {
		...
		if (isSelfEdge(edge)) {
			...
			pContext.strokeArc((int) arc.getCenterX(), (int) arc.getCenterY(), (int) arc.getRadiusX(),
					(int) arc.getStartAngle(), (int) arc.getLength(), ColorScheme.get().stroke());
		}
		else {
		...
		if (isSelfEdge(pEdge)) {
			...
			if (getPosition(pEdge) == 1) {
				ArrowHeadRenderer.draw(pContext, ArrowHead.V,
						new Point(connectionPoint2.x() + SELF_EDGE_OFFSET, connectionPoint2.y() - SELF_EDGE_OFFSET / 4),
						getConnectionPoints(pEdge).point2());
			}
			else {
				ArrowHeadRenderer.draw(pContext, ArrowHead.V,
						new Point(connectionPoint2.x() - SELF_EDGE_OFFSET / 4, connectionPoint2.y() - SELF_EDGE_OFFSET),
						getConnectionPoints(pEdge).point2());
			}
		}
		else {
			ArrowHeadRenderer.draw(pContext, ArrowHead.V, getControlPoint(pEdge), getConnectionPoints(pEdge).point2());
		}
	}
	...
	private Rectangle getNormalEdgeLabelBounds(StateTransitionEdge pEdge) {
		...
		int gap = 3;
		if (line.y1() >= line.y2() - VERTICAL_TOLERANCE && line.y1() <= line.y2() + VERTICAL_TOLERANCE) {
	...
	/*
	 * Positions the label above the self edge, centered in the middle of it.
	 * 
	 * @return the bounds of the label text
	 */
	private Rectangle getSelfEdgeLabelBounds(StateTransitionEdge pEdge) {
		...
		if (getPosition(pEdge) == 1) {
			return new Rectangle(line.x1() + SELF_EDGE_OFFSET - textDimensions.width() / 2,
					line.y1() - SELF_EDGE_OFFSET - textDimensions.height(), textDimensions.width(),
	...
	private Arc getSelfEdgeShape(Edge pEdge) {
		/*
		 * The starting position is determined from the bottom middle (0 degrees) and
		 * going counter-clockwise. The extent runs clockwise.
		 */
	...
	/**
	 * @return An index that represents the position in the list of edges between
	...
	private Path getNormalEdgeShape(Edge pEdge) {
		...
		QuadCurveTo curveTo = new QuadCurveTo(getControlPoint(pEdge).x(), getControlPoint(pEdge).y(), line.point2().x(),
				line.point2().y());
	...
	private Line getNormalEdgeConnectionsPoints(Edge pEdge) {
		...
		return new Line(parent().getConnectionPoints(pEdge.start(), d1), parent().getConnectionPoints(pEdge.end(), d2));
	}

	@Override
	public Canvas createIcon(DiagramType pDiagramType, DiagramElement pElement) { // CSOFF: Magic numbers
