...
public abstract class AbstractEdgeRenderer implements EdgeRenderer {
	...
	/**
	 * The default behavior is to draw a straight line between the connections
	 * points oriented in the direction of each other node.
	 * 
	...
	/*
	 * The default behavior implemented by this method is to find the connection
	 * point that each start/end node provides for a direction that is oriented
	 * following a straight line connecting the center of the rectangular bounds for
	 * each node.
	 */
	@Override
	...
	protected String wrapLabel(String pString, int pDistanceInX, int pDistanceInY) {
		...
		if (distanceInX > 0) {
			...
			lineLength = Math.max(MAX_LENGTH_FOR_NORMAL_FONT,
					(int) ((distanceInX / 4) * (1 - angleInDegrees / DEGREES_180)));
		}
