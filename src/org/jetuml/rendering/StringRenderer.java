...
/**
 * A class to render strings with various decorations: underline, bold, with
...
public final class StringRenderer {
	...
	public void draw(String pString, Rectangle pBoundingBox, RenderingContext pContext) {
		...
		for (String line : pString.split("\n")) {
			drawSingleLine(line, new Rectangle(pBoundingBox.x(), currentY, pBoundingBox.width(), lineHeight()),
					pContext);
			currentY += lineHeight();
		}
	}
	...
	private void drawSingleLine(String pString, Rectangle pBoundingBox, RenderingContext pContext) {
		...
		pContext.drawText(pString, pBoundingBox, aAlign, ColorScheme.get().stroke(), font(), fontDimension());
	...
	/**
	 * Gets the width and height required to show pString, including padding around
	 * the string.
	 * 
