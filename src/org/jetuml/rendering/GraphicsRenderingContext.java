...
/**
 * Wrapper for a graphics context to serve as a target for all drawing
 * operations.
 * 
 ...
 * 
 * Only a single RenderingContext should ever be associated with any
 * GraphicsContext.
 */
public class GraphicsRenderingContext implements RenderingContext {
	...
	/**
	 * Creates a rendering context that draws on the provided graphics context.
	 * 
	...
	public void strokeArc(int pCenterX, int pCenterY, int pRadius, int pStartAngle, int pLength, Color pStrokeColor) {
		...
		aContext.save();
		aContext.setStroke(pStrokeColor);
		aContext.strokeArc(pCenterX - pRadius, pCenterY - pRadius, pRadius * 2, pRadius * 2, pStartAngle, pLength,
				ArcType.OPEN);
		aContext.restore();
	}
	...
	private void strokePath(Path pPath, boolean pFill) {
		...
		for (PathElement element : pPath.getElements()) {
			if (element instanceof MoveTo moveTo) {
				...
				aContext.quadraticCurveTo(curve.getControlX(), curve.getControlY(), curve.getX(), curve.getY());
			}
		}
		if (pFill) {
			aContext.fill();
	...
	public void drawRoundedRectangle(Rectangle pRectangle, Color pFillColor, Color pStrokeColor,
		...
		pDropShadow.ifPresent(shadow -> aContext.setEffect(shadow));
		aContext.fillRoundRect(pRectangle.x(), pRectangle.y(), pRectangle.width(), pRectangle.height(),
				ROUNDED_RECTANGLE_ARC, ROUNDED_RECTANGLE_ARC);
		pDropShadow.ifPresent(shadow -> aContext.setEffect(null));
		aContext.strokeRoundRect(pRectangle.x(), pRectangle.y(), pRectangle.width(), pRectangle.height(),
				ROUNDED_RECTANGLE_ARC, ROUNDED_RECTANGLE_ARC);
		aContext.restore();
	}
