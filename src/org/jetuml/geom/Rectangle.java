...
/**
 * A framework independent representation of a rectangle in 2-dimensional
 * integer space.
 * 
...
public record Rectangle(int x, int y, int width, int height) {
	...
	public boolean contains(Point pPoint) {
		assert pPoint != null;
		return pPoint.x() >= x && pPoint.x() <= maxX() && pPoint.y() >= y && pPoint.y() <= maxY();
	}
	...
	public boolean contains(Rectangle pRectangle) {
		...
		return pRectangle.x >= x && pRectangle.y >= y && pRectangle.maxX() <= maxX() && pRectangle.maxY() <= maxY();
	}
	 ...
	 * @return A new rectangle that is this rectangle enlarged to include
	 *     pPoint.
	 * @pre pPoint != null
	 */
