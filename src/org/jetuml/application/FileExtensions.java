...
/**
 * A utility class to create and access diagram extension filters used by the
 * file chooser.
 */
public final class FileExtensions {
	...
	private static final ExtensionFilter FILTER_APPLICATION = new ExtensionFilter(
			RESOURCES.getString("application.file.name"), "*" + EXTENSION_JET);
	private static final ExtensionFilter FILTER_ALL = new ExtensionFilter(RESOURCES.getString("application.file.all"),
			"*.*");
	...
	/**
	 * @return An unmodifiable list of all filters applicable to the
	 *     application. This list includes one filter for each diagram type, the
	 *     general filter for all file types, and the "application" filter for all
	 *     diagram file types. Never null.
	 */
	public static List<ExtensionFilter> all() {
		List<ExtensionFilter> result = aExtensionFilters.entrySet().stream().sorted(comparingByKey())
	 ...
	 * @return A file with the same name as pFile, but with the application
	 *     extension removed. If there is no application extension to clip, the same
	...
	public static File clipApplicationExtension(File pFile) {
		...
		return new File(
				pFile.getAbsolutePath().substring(0, pFile.getAbsolutePath().length() - EXTENSION_JET.length()));
	}
}
