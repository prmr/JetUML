...
/**
 * A Singleton that manages all user preferences global to the application.
 */
@Singleton
public final class UserPreferences {
	...
	public enum BooleanPreference {
		...
		showGrid(true), showToolHints(false), autoEditNode(false), verboseToolTips(false), showTips(true), darkMode(
				false);
	...
	private UserPreferences() {
		for (BooleanPreference preference : BooleanPreference.values()) {
			aBooleanPreferences.put(preference, Boolean.valueOf(
					Preferences.userNodeForPackage(JetUML.class).get(preference.name(), preference.getDefault())));
		}
		for (IntegerPreference preference : IntegerPreference.values()) {
			aIntegerPreferences.put(preference, Integer.valueOf(
					Preferences.userNodeForPackage(JetUML.class).get(preference.name(), preference.getDefault())));
		}
	...
	/**
	 * Adds a handler for a boolean property change. Don't forget to remove
	 * handers if objects are removed, e.g., diagram Tabs.
	 * 
	...
	/**
	 * Adds a handler for an integer property change. Don't forget to remove
	 * handers if objects are removed, e.g., diagram Tabs.
	 * 
