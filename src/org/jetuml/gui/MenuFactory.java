...
/**
 * A class for creating menus from strings in a resource bundle. The class does
 * not depend on the Singleton ApplicationResource instance to improve
 * testability.
 * 
 * An instance of this class is intended to be initialized with a resource
 * bundle, used to create any number of menus, then discarded.
 * 
 * The user data for a menu item is a boolean flag that indicates whether the
 * menu is only relevant to a current diagram (true) or generally relevant
 * (false).
 * 
 * To create a menu item, the creation methods will look for the following
 * resources in the specified ApplicationResources object, with a key followed
 * by a prefix P.
 * 
 * - P.text (required): The text of the menu - P.mnemonic (optional): The single
 * letter mnemonic for opening the menu with the keyboard - P.icon (optional):
 * The path to the menu icon - P.accelerator (optional): The shortcut key
 * combination. Add ".mac" for macs.
 */
class MenuFactory {
	 ...
	 * 
	 * @param pPrefix A string such as "file.open" that indicates the
	 *     menu->submenu path
	 * @param pHandler The callback to execute when the menu item is selected.
	 * @param pDiagramSpecific True if the action requires a diagram tab to be
	 *     active. A value of false will indicate that the menu should be
	 *     disabled if the welcome tab is showing. See setMenuVisibility
	 ...
	 * @return A menu item for the action described.
	 */
	public MenuItem createCheckMenuItem(String pPrefix, boolean pDiagramSpecific, boolean pInitialState,
			EventHandler<ActionEvent> pHandler) {
	...
	/*
	 * Initializes pMenuItem with text, mnemonic, accelerator, etc., and returns
	 * it.
	 */
	private void initialize(MenuItem pMenuItem, String pPrefix) {
		...
		if (IS_MAC && aResources.containsKey(pPrefix + KEY_ACCELERATOR_MAC)) {
			pMenuItem
					.setAccelerator(KeyCombination.keyCombination(aResources.getString(pPrefix + KEY_ACCELERATOR_MAC)));
		}
	 ...
	 * 
	 * @return A new text string with the character '_' before the letter that
	 * is to be the mnemonic for the menu item. The resulting string is intended
	 * to be set as the text of the menu. If pMnemonic is not a single letter
	 * that is part of pText, it is simply ignored.
	 */
	 ...
	 * 
	 * @param pPrefix A string such as "file" that indicates where to find the
	 *     related resources in the resource bundle.
