...
/**
 * A tool bar than contains various tools and command shortcut buttons. Only one
 * tool can be selected at the time. The tool bar also controls a pop-up menu
 * with the same tools as the tool bar. Labels can optionally be shown next to
 * tools.
 */
public class DiagramTabToolBar extends ToolBar implements BooleanPreferenceChangeHandler {
	 ...
	 * 
	 * @param pDiagramRenderer The renderer for the diagram associated with this
	 *     tool bar.
	 */
	public DiagramTabToolBar(DiagramRenderer pDiagramRenderer) {
		...
		// Method setToolToBeSelect assumes the selection tool will always be
		// the first button in the toggle group.
	...
	private void installCopyToClipboard() {
		...
		assert getItems().size() > 0; // We copy size information from the top
										// button
	 ...
	 * selected button, if available. A tool is unavailable if the select tool
	 ...
	 * 
	 * @param pScreenXCoordinate The X-coordinate where to position the menu, in
	 *     screen coordinates.
	 * @param pScreenYCoordinate The Y-coordinate where to position the menu, in
	 *     screen coordinates.
	 */
	...
	/**
	 * Sets the selected tool to be the one at pIndex (zero-indexed) in the tool
	 * group. Does nothing if there is no tool at this index.
	 * 
	...
	private void showButtonLabels(boolean pShow) {
		for (javafx.scene.Node item : getItems()) {
			...
			if (pShow) {
				if (item instanceof SelectableToolButton toolButton && toolButton.getPrototype().isPresent()) {
	...
	/**
	 * Recreates the tool bar button and pop-up menu icons when turning dark
	 * mode on or off.
	 */
