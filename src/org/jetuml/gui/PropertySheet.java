...
/**
 * A layout that presents the properties of a DiagramElement and allow editing
 * them.
 */
public class PropertySheet extends GridPane {

	private static final KeyCombination STEREOTYPE_DELIMITER_TRIGGER = new KeyCodeCombination(KeyCode.Q,
			KeyCombination.CONTROL_DOWN);

	/**
	 * A handler for whenever a property is being detected as being edited. This
	 * allows a more responsive UI, where properties can be shown as they are
	 * typed, as opposed to only when the value is entered.
	 */
	...
	/*
	 * Special case due to the poor decision in the past to declare NodeNodes as
	 * a subclass of Name nodes, which means that its control would use the NAME
	 * property and thus be labeled with "Name" instead of "Text". Ideally, the
	 * property for NodeNodes should be a new enum called TEXT, and NoteNode
	 * should not be children of NamedNodes. However, changing this would break
	 * backward compatibility. TODO: Next time we do a backward-incompatible
	 * release, we'll fix this.
	 */
	...
	/*
	 * Not the greatest but avoids over-engineering the rest of the properties
	 * API. CSOFF:
	 */
	private boolean extended(PropertyName pProperty) {
		return aElement.getClass() == ClassNode.class || aElement.getClass() == InterfaceNode.class ||
	...
	/*
	 * Add a feature to the control that adds the stereotype delimiters to the
	 * input control if Ctrl-Q is typed, and position the caret between the
	 * delimiters.
	 */
	...
	private static void addTabbingFeature(TextArea pTextArea) {
		pTextArea.addEventFilter(KeyEvent.KEY_PRESSED, pKeyEvent -> {
			...
			if (pKeyEvent.isAltDown() || pKeyEvent.isMetaDown() || pKeyEvent.isShiftDown() ||
					!(pKeyEvent.getSource() instanceof TextArea)) {
				return;
			}
			...
			if (pKeyEvent.isControlDown()) {
				...
				pKeyEvent.consume();
				final KeyEvent tabControlEvent = new KeyEvent(pKeyEvent.getSource(), pKeyEvent.getTarget(),
						pKeyEvent.getEventType(), pKeyEvent.getCharacter(), aFocusEventText, pKeyEvent.getCode(),
						pKeyEvent.isShiftDown(), true, pKeyEvent.isAltDown(), pKeyEvent.isMetaDown());
