...
/**
 * A modal dialog that allows the user to edit the properties of a
 * DiagramElement.
 */
public class PropertyEditorDialog {
	 ...
	 * @param pPropertyChangeListener A callback to run whenever a property
	 *     changes.
	 */
	 ...
	 * 
	 * @return A compound operation that represents the modification of each
	 *     property modified through this dialog.
	 */
	...
	private static class PropertyChangeTracker {
		...
		/**
		 * Creates and returns a CompoundOperation that represents any change in
		 * properties detected between the time startTracking and stopTracking
		 * were called.
		 * 
		...
		public CompoundOperation stopTracking() {
			...
			for (Property property : aProperties) {
				if (!aOldValues.get(property.name()).equals(property.get())) {
					...
					operation.add(new SimpleOperation(() -> property.set(newValue), () -> property.set(oldValue)));
				}
			}
			return operation;
		}
