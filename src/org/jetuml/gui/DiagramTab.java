...
public class DiagramTab extends Tab implements KeyEventHandler {
	...
	public DiagramTab(Diagram pDiagram) {
		...
		// being able to
		// decorate it with CSS. The StackPane needs to have a fixed size so the
		// border fits the
		...
		// First, wrap the StackPane in a Group to allow the scrolling to be
		// based around the visual bounds
		// of the canvas rather than its layout bounds.
		// Then we wrap the Group within an additional, resizable StackPane that
		// can grow to fit the parent
		// ScrollPane and thus center the decorated canvas.
		ScrollPane scroll = new ScrollPane(new StackPane(new Group(pane)));

		// The call below is necessary to removes the focus highlight around the
		// Canvas
		// See issue #250
	...
	public void close() {
		...
		UserPreferences.instance()
				.removeBooleanPreferenceChangeHandler((DiagramTabToolBar) ((BorderPane) getContent()).getRight());
	...
	/**
	 * @return True if the diagram in this tab has unsaved changes.
	 */
	...
	/*
	 * Fetches the ScrollPane component that wraps the canvas from the scene
	 * graph
	 */
	...
	private ViewportProjection getViewportProjection() {
		...
		// Because, when the scrollbars are not displayed, the Scrollpane will
		// increase
		// the viewport size beyond the canvas size, it's necessary to max out
		// the dimensions
		...
		return new ViewportProjection(viewportWidth, viewportHeight, (int) aDiagramCanvas.getWidth(),
	...
	/*
	 * Converts the key typed to an 1-based index that represents the tool to
	 * select in the toolbar. The Keys 1-0 map to 1-10, then a maps to 11, b to
	 * 12, etc. Capitalization does not matter. Returns -1 is the key is not in
	 * a range between 0 and Z. CSOFF:
	 */
	...
	@Override
	public void keyTyped(String pChar) { // -1 because the input is 1-index and
											// setSelectedTool is 0-indexed
