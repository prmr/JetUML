...
public class DiagramCanvas extends Canvas implements SelectionObserver, BooleanPreferenceChangeHandler,
		IntegerPreferenceChangeHandler, StringPreferenceChangeHandler {

	private static final double LINE_WIDTH = 0.6;
	/*
	 * The number of pixels to leave around a diagram when the canvas size is
	 * automatically increased to accommodate a diagram larger than the
	 * preferred size.
	 */
	...
	/**
	 * @return True if the diagram controlled by this controller has unsaved
	 *     changes.
	 */
	...
	/**
	 * Pastes the content of the clip board into the graph managed by this
	 * panel.
	 */
	public void paste() {
	...
	public void editSelected() {
		...
		if (edited.isPresent()) {
			PropertyEditorDialog dialog = new PropertyEditorDialog(
					((EditorFrame) getScene().getRoot()).getDialogStage(), edited.get(), () -> paintPanel());
	...
	/**
	 * Paints the panel and all the graph elements in aDiagramView. Called after
	 * the panel is resized.
	 */
	public void paintPanel() {
		Rectangle area = new Rectangle(0, 0, width(), height());
		aRenderingContext.drawRectangle(area, ColorScheme.get().background(), ColorScheme.get().background(),
				Optional.empty());
	...
	private void drawHandlesOnSelectedEdges() {
		aSelected.stream().filter(Edge.class::isInstance).map(Edge.class::cast).forEach(
				edge -> aAccessoriesRenderer.drawHandles(aDiagramBuilder.renderer().getConnectionPoints(edge)));
	}

	private void drawHandlesOnSelectedNodes() {
		aSelected.stream().filter(Node.class::isInstance).map(Node.class::cast)
				.forEach(node -> aAccessoriesRenderer.drawHandles(aDiagramBuilder.renderer().getBounds(node)));
	}

	/**
	 * Undoes the most recent command. If the UndoManager performs a command,
	 * the method it calls will repaint on its own
	 */
	public void undo() {
	...
	/**
	 * Removes the last undone action and performs it. If the UndoManager
	 * performs a command, the method it calls will repaint on its own
	 */
	public void redo() {
	...
	/**
	 * When the shift key is pressed on a class diagram, perform node attachment
	 * or detachment if possible.
	 */
	public void shiftKeyPressed() {
		...
		if (((ClassDiagramBuilder) aDiagramBuilder).canLinkToPackage(selectedNodes)) {
			aProcessor.executeNewOperation(
					((ClassDiagramBuilder) aDiagramBuilder).createLinkToPackageOperation(selectedNodes));
		}
		else if (((ClassDiagramBuilder) aDiagramBuilder).canUnlinkFromPackage(selectedNodes)) {
			aProcessor.executeNewOperation(
					((ClassDiagramBuilder) aDiagramBuilder).createUnlinkFromPackageOperation(selectedNodes));
		}
	...
	/*
	 * If the diagram is smaller than the preferred dimension, return the
	 * preferred dimension. Otherwise, grow the dimensions to accommodate the
	 * diagram.
	 */
	private Dimension getDiagramCanvasWidth() {
		Rectangle bounds = aDiagramBuilder.renderer().getBounds();
		return new Dimension(Math.max(getPreferredDiagramWidth(), bounds.maxX() + DIMENSION_BUFFER),
	...
	private Rectangle computeLasso() {
		return new Rectangle(Math.min(aMouseDownPoint.x(), aLastMousePoint.x()),
				Math.min(aMouseDownPoint.y(), aLastMousePoint.y()), Math.abs(aMouseDownPoint.x() - aLastMousePoint.x()),
	...
	private Optional<? extends DiagramElement> getSelectedElement(MouseEvent pEvent) {
		...
		Optional<? extends DiagramElement> element = aDiagramBuilder.renderer().edgeAt(mousePoint);
		if (!element.isPresent()) {
			element = aDiagramBuilder.renderer().selectableNodeAt(new Point(mousePoint.x(), mousePoint.y()));
		}
		return element;
	}
	...
	private void handleSelection(MouseEvent pEvent) {
		...
		if (element.isPresent()) {
			if (pEvent.isControlDown()) {
				...
				// The test is necessary to ensure we don't undo multiple
				// selections
	...
	/*
	 * The creation of a node is not allowed if there is an element under the
	 * mouse, except if the target node is a PackageNode OR the tool is a field
	 * node.
	 */
	...
	private void handleNodeCreation(MouseEvent pEvent) {
		...
		aProcessor
				.executeNewOperation(aDiagramBuilder.createAddNodeOperation(newNode, new Point(point.x(), point.y())));
	...
	/*
	 * Move by a delta that will align the result of the move gesture with the
	 * grid.
	 */
	private void alignMoveToGrid() {
		...
		if (selectedNodes.hasNext()) {
			...
			// ensure the bounds of the entire selection are not outside the
			// walls of the canvas
	...
	private void releaseRubberband(Point pMousePoint) {
		...
		if (pMousePoint.distance(aMouseDownPoint) > CONNECT_THRESHOLD) {
			aProcessor
					.executeNewOperation(aDiagramBuilder.createAddEdgeOperation(newEdge, aMouseDownPoint, pMousePoint));
			Optional<Violation> violation = aDiagramValidator.validate();
	...
	// This will be doable by collecting all edges connected to a transitively
	// selected node.
	private void moveSelection(Point pMousePoint) {
		...
		// If this translation results in exceeding the canvas bounds, roll
		// back.
		Rectangle bounds = aDiagramBuilder.renderer().getBoundsIncludingParents(aSelected);
		int dxCorrection = Math.max(-bounds.x(), 0) + Math.min(width() - bounds.maxX(), 0);
		int dyCorrection = Math.max(-bounds.y(), 0) + Math.min(height() - bounds.maxY(), 0);
	...
	public Image createImage() {
		Rectangle bounds = aDiagramBuilder.renderer().getBounds();
		Canvas canvas = new Canvas(bounds.width() + DIAGRAM_PADDING * 2, bounds.height() + DIAGRAM_PADDING * 2);
	...
	/**
	 * @return A string that contains an SVG description of the rendered
	 *     diagram.
	 */
	...
	private List<Node> selectedNodes() {
		return aSelected.stream().filter(e -> Node.class.isAssignableFrom(e.getClass())).map(Node.class::cast)
				.collect(toList());
	}

	/**
	 * Records information about an active lasso selection tool, select all
	 * elements in the lasso, and triggers a notification.
	 * 
	...
	/**
	 * Records information about an active rubberband selection tool and
	...
	/**
	 * Adds an element to the selection set and sets it as the last selected
	 * element. If the element is already in the list, it is added to the end of
	 * the list. If the node is transitively a child of any node in the list, it
	 * is not added. Triggers a notification.
	 * 
	...
	private void internalAddToSelection(DiagramElement pElement) {
		if (!containsParent(pElement)) {
			...
			for (DiagramElement element : toRemove) {
				// Do no use removeFromSelection because it notifies the
				// observer
	...
	/**
	 * Removes pElement from the list of selected elements, or does nothing if
	 * pElement is not selected. Triggers a notification.
	 * 
	...
	/**
	 * Sets pElement as the single selected element. Triggers a notification.
	 * 
