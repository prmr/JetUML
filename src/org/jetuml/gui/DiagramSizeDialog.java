...
/**
 * A modal dialog that allows users to set the size of the future diagrams to be
 * created.
 */
public class DiagramSizeDialog {
	...
	private Pane createButtons() {
		...
		ok.setOnAction(pEvent -> {
			if (isValidDiagramDimensionSize(aWidthField.getText()) &&
					isValidDiagramDimensionSize(aHeightField.getText())) {
				UserPreferences.instance().setInteger(IntegerPreference.diagramWidth,
						Integer.parseInt(aWidthField.getText()));
				UserPreferences.instance().setInteger(IntegerPreference.diagramHeight,
						Integer.parseInt(aHeightField.getText()));
				aStage.close();
			}
			else {
				showInvalidSizeAlert();
