...
/**
 * A modal dialog that allows users to change the font and font size of text in
 * diagrams.
 */
public class FontDialog {
	...
	private void prepareStage() {
		...
		aStage.setOnCloseRequest(pEvent -> {
			restoreUserSettings();
			aStage.setOnCloseRequest(null); // So the handler does not persist
	...
	private void onInput() {
		if (!aFonts.getSelectionModel().getSelectedItem().equals(getCurrentFont())) {
			UserPreferences.instance().setString(StringPreference.fontName,
					aFonts.getSelectionModel().getSelectedItem());
		}

		if (aSizes.getSelectionModel().getSelectedItem() != getCurrentFontSize()) {
			UserPreferences.instance().setInteger(IntegerPreference.fontSize,
					aSizes.getSelectionModel().getSelectedItem());
		}
	}
