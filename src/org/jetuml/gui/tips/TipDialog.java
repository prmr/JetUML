...
public class TipDialog {
	...
	public TipDialog(Stage pDialogStage) {
		...
		aShowTipsOnStartupCheckBox
				.setSelected(UserPreferences.instance().getBoolean(UserPreferences.BooleanPreference.showTips));
		aShowTipsOnStartupCheckBox.setOnAction(e -> UserPreferences.instance()
				.setBoolean(UserPreferences.BooleanPreference.showTips, aShowTipsOnStartupCheckBox.isSelected()));
	}

	/**
	 * Shows the tip dialog and blocks the remainder of the UI until the tip dialog
	 * is closed.
	 */
	public void show() {
		prepareStage();
	...
	/**
	 * Returns a Node that presents the content of a TipElement. Remark that the
	 * preconditions will change if new media are used in TipElements
	 * 
	 * @param pTipElement the TipElement to get as a Node
	 * @param pParent     the VBox that will eventually contain the tipElement. This
	 *                    parameter is necessary to ensure that dimensions and
	 *                    properties of the TipElement Nodes such as text wrapping
	 ...
	 *      pTipElement.getMedia().equals(Media.IMAGE)
	 */
	...
	private static ImageView getImageTipElementAsImageView(TipElement pTipElement) {
		...
		try (InputStream inputStream = TipDialog.class.getResourceAsStream(tipImagesDir + "/" + imageName)) {
			...
			if (imageNode.getImage().getWidth() > WINDOW_PREF_WIDTH - 2 * PADDING - 4 * DEFAULT_NODE_SPACING) {
				...
				// two times the padding because of the VBox padding, and a bit extra to make up
				// for
