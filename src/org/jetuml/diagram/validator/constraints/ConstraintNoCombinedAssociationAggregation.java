...
public final class ConstraintNoCombinedAssociationAggregation implements EdgeConstraint {
	...
	public boolean satisfied(Edge pEdge, Diagram pDiagram) {
		return pDiagram.edges().stream().filter(ConstraintNoCombinedAssociationAggregation::isAssociationOrAggregation)
				.filter(edge -> isBetweenSameNodes(edge, pEdge)).count() <= 1;
	}
