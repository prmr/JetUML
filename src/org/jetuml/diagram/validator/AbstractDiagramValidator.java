...
abstract class AbstractDiagramValidator implements DiagramValidator {
	...
	private static final Set<Class<? extends Node>> UNIVERSAL_NODES_TYPES = Set.of(PointNode.class, NoteNode.class);
	private static final Set<Class<? extends Edge>> UNIVERSAL_EDGES_TYPES = Set.of(NoteEdge.class);
	private static final Set<EdgeConstraint> UNIVERSAL_CONSTRAINTS = Set.of(new ConstraintValidNoteEdge(),
			new ConstraintNoEdgeToPointExceptNoteEdge(), new ConstraintNoEdgeToNoteExceptNoteEdge(),
	 ...
	 * @param pValidNodeTypes The node types valid for this diagram, in addition
	 *     to the universal nodes valid by default.
	 * @param pValidEdgeTypes The edge types valid for this diagram, in addition
	 *     to the universal edges valid by default.
	 * @pre pDiagram != null && pValidNodeTypes != null && pValidEdgeTypes !=
	 *     null;
	 */
	...
	public final Optional<Violation> validate() {
		return validateElementTypes().or(this::validatePointNodes).or(this::validateDiagramNodes)
	...
	private Optional<Violation> validateSemantics() {
		return aDiagram.edges().stream().map(edge -> validateAllConstraintsFor(edge)).filter(Optional::isPresent)
	...
	private Optional<Violation> validateAllConstraintsFor(Edge pEdge) {
		...
		return aConstraints.stream().filter(constraint -> !constraint.satisfied(pEdge, aDiagram)).findFirst()
				.map(constraint -> Violation.newSemanticViolation(constraint));
	}

	private boolean hasValidElementTypes() {
		return aDiagram.allNodes().stream().allMatch(node -> aValidNodeTypes.contains(node.getClass())) && 
				aDiagram.edges().stream().allMatch(edge -> aValidEdgeTypes.contains(edge.getClass()));
	}
	...
	private boolean hasValidPointNodes() {
		return diagram().rootNodes().stream().filter(PointNode.class::isInstance)
				.allMatch(node -> diagram().edgesConnectedTo(node).iterator().hasNext());
	}

	/**
	 * Step method in the template method design pattern to allow processing
	 * diagram-specific node validation.
	 */
