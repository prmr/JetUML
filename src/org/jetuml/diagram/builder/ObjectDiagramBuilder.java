...
public class ObjectDiagramBuilder extends DiagramBuilder {
	...
	protected void completeEdgeAdditionOperation(CompoundOperation pOperation, Edge pEdge, Node pStartNode,
		...
		if (pStartNode.getClass() == FieldNode.class) {
			...
			pOperation.add(new SimpleOperation(() -> node.setValue(""), () -> node.setValue(oldValue)));
		}
	}

	/*
	 * Find if the node to be added can be added to an object. Returns null if
	 * not. If a node is already the parent of the field (from a previously
	 * undone operation), return this node. Otherwise, find if a node is at the
	 * point
	 */
	private ObjectNode findObject(FieldNode pNode, Point pPoint) {
		...
		for (Node node : aDiagramRenderer.diagram().rootNodes()) {
			if (node == pNode) {
				...
				candidates.add((ObjectNode) node); // canAddNodeAsChild ensures
													// the downcast is valid
			}
		}
		// Pick the last node in the list as some inexact but simple
		// heuristic for choosing the top node. We'll need a z-coordinate to do
		// better.
	...
	public DiagramOperation createAddNodeOperation(Node pNode, Point pRequestedPosition) {
		...
		if (pNode instanceof FieldNode fieldNode) {
			...
			if (object != null) {
				result = new SimpleOperation(() -> object.addChild(pNode), () -> object.removeChild(pNode));
			}
		}
		if (result == null) {
