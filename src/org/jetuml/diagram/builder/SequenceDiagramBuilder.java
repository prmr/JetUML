...
public class SequenceDiagramBuilder extends DiagramBuilder {
	...
	protected List<DiagramElement> getCoRemovals(DiagramElement pElement) {
		...
		// Implicit parameter nodes downstream of constructor calls should not
		// be removed
	...
	protected void completeEdgeAdditionOperation(CompoundOperation pOperation, Edge pEdge, Node pStartNode,
		...
		if (start.getClass() == ImplicitParameterNode.class) {
			...
			pOperation.add(
					new SimpleOperation(() -> parent.addChild(newCallNode), () -> parent.removeChild(newCallNode)));
			start = newCallNode;
		}
		...
		pOperation.add(new SimpleOperation(() -> parent.addChild(end), () -> parent.removeChild(end)));
	...
	/**
	 * @return True if the start end node can support the addition of a call
	 *     edge.
	 */
	...
	public DiagramOperation createAddNodeOperation(Node pNode, Point pRequestedPosition) {
		...
		if (pNode instanceof CallNode) {
			...
			if (target.isPresent()) {
				result = new SimpleOperation(() -> target.get().addChild(pNode), () -> target.get().removeChild(pNode));
			}
		}
		if (result == null) {
	...
	/*
	 * If pPoint is inside an ImplicitParameterNode but below its top rectangle,
	 * returns the Optional value of that node. Otherwise, returns
	 * Optional.empty().
	 */
	...
	/**
	 * @param pElements The DiagramElements to obtain the corresponding
	 *     ReturnEdges for.
	...
	private Optional<Edge> getReturnEdge(Edge pEdge) {
		return renderer().diagram().edges().stream().filter(ReturnEdge.class::isInstance)
				.filter(edge -> edge.start() == pEdge.end()).filter(edge -> edge.end() == pEdge.start()).findFirst();
	}

	/**
	 * @param pNode The node to check.
	 * @return True if pNode is a CallNode and is at the end of a
	 *     ConstructorEdge.
	 */
	...
	/**
	 * @param pNode The Node to obtain the caller and upstream DiagramElements
	 *     for.
	 * @return The Collection of DiagramElements in the upstream of pNode.
	 *     Excludes pNode and elements returned by getCoRemovals method in the
	...
	private Collection<DiagramElement> getNodeUpstreams(Node pNode) {
		...
		if (pNode.getClass() == CallNode.class) {
			...
			if (caller.isPresent() && getCaller(caller.get()).isEmpty() && 
					onlyCallsToASingleImplicitParameterNode(caller.get(), pNode)) {
	...
	/*
	 * Returns true if pNode is the ImplicitParameterNode that gets created in
	 * constructor call
	 */
	...
	/**
	 * Check if the start node is either a CallNode or ImplicitParameterNode,
	 * and the end node is an ImplicitParameterNode with no child nodes. The end
	 * point of the edge should land on the top rectangle of the end Node.
	 * 
	 * @param pStart the start position of the mouse.
	 * @param pEnd the end position of the mouse.
	 * @return True if the start node and the end node of the edge satisfy the
	 *     conditions to create the constructor call.
	...
	private boolean canCreateConstructorCall(Point pStart, Point pEnd) {
		...
		if (start.get().getClass() != ImplicitParameterNode.class && start.get().getClass() != CallNode.class) {
			return false;
		}
