...
/**
 * Parses any value in a JSON document according to the ECMA-404 2nd edition
 * December 2017.
 */
final class JsonAnyValueParser implements JsonValueParser {
	private static final List<JsonValueParser> PARSERS = List.of(new JsonStringParser(), new JsonBooleanParser(),
			new JsonIntegerParser(), new JsonObjectParser(), new JsonArrayParser());

	@Override
	public boolean isApplicable(ParsableCharacterBuffer pInput) {
		return PARSERS.stream().anyMatch(parser -> parser.isApplicable(pInput));
	}
