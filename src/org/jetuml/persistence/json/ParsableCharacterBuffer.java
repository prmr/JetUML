...
/**
 * An extension of CharacterBuffer that supports basic parsing operations. In
 * contrast to the CharacterBuffer, which has strict preconditions for
 * state-changing methods, this class accepts any input and throws a
 * JsonParsingException if the operation is illegal.
 */
class ParsableCharacterBuffer extends CharacterBuffer {
	...
	/**
	 * Read the next character in the buffer and check that it is the same as
	 * pCharacter. Throws a JsonException if it is not or if there are no more
	 * characters to read.
	 * 
