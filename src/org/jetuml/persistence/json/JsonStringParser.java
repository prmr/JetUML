...
/**
 * Parses strings in JSON document according to the ECMA-404 2nd edition
 * December 2017. Also provides support for writing strings in JSON standard.
 */
final class JsonStringParser implements JsonValueParser {
	 ...
	 * starting at the next available position, or throws an exception. This method
	 * has a side effect on the character buffer. For this method to succeed, the
	 * first character in the buffer must be the escape character '\\' (\u005C). The
	 * buffer will be left at the position of the last character in the escape.
	 * 
	 * @param pInput A buffer from which to read an escape.
	 * 
	 * @return A character that corresponds to the next escaped character read from
	 * pInput
	 * 
	 * @pre pInput != null
	 * 
	 * @throws JsonException if it is not possible to parse an escape from pInput.
	 */
	...
	/*
	 * Attempts to retrieve a valid unicode value by reading characters starting at
	 * the next available position, or throws an exception. This method has a side
	 * effect on the character buffer. For this method to succeed, the first
	 * character in the buffer must be a hexadecimal digit (0-A) in either lower or
	 * upper case. The buffer will be left at the position of the last character in
	 * the escape.
	 * 
	 * @param pInput A buffer from which to read a four-digit hexadecimal value.
	 * 
	 * @return A character that corresponds to the unicode code point represented by
	 * the escape.
	 * 
	 * @pre pInput != null
	 * 
	...
	/**
	 * To be applicable the next character must be a quote, i.e., the opening quote
	 * for the string.
	 */
	@Override
	...
	/**
	 * Attempts to retrieve a valid JSON string from pBuffer by reading characters
	 * starting at the next available position, or throws an exception. This method
	 * has a side effect on the character buffer. For this method to succeed, the
	 * first character in the buffer must be the quote character '"' (\u0022), but
	 * it is not necessary for the last character to also be a quote. Characters
	 * beyond the closing quote will simply not be read. The buffer will be left at
	 * the position of the closing quote.
	 * 
	 * @param pInput A buffer from which to read a string.
	 * @return A valid string
	 * @pre pInput != null
	 * @throws JsonException if it is not possible to parse a string from pInput.
	 */
	@Override
	...
	/**
	 * Escapes characters that need to be escaped so that the string is a proper
	 * JSON format string.
	 * 
