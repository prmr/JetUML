...
/**
 * Utility methods to uniformly validate and access Object instances as Json
 * values.
 */
final class JsonValueValidator {
	private static final Set<Class<?>> VALID_VALUE_TYPES = Set.of(String.class, Boolean.class, Integer.class,
	...
	/**
	 * Validates that pValue is a non-null instance that represents a valid JSON
	 * value: String, Boolean, Integer, JsonObject, or JsonArray.
	 * 
	 * @param pValue The value to check. If null, throws a JsonException
	 * @throws JsonException is pValue is null or an instance of an object not
	 *                       recognized as a valid JSON value.
	 */
	...
	static int asInt(Object pValue) {
		if (pValue.getClass() != Integer.class) {
			throw new JsonException(
					String.format("Attempting to retrieve an %s as an int", pValue.getClass().getSimpleName()));
		}
		return (int) pValue;
	}
	...
	static String asString(Object pValue) {
		if (pValue.getClass() != String.class) {
			throw new JsonException(
					String.format("Attempting to retrieve an %s as a String", pValue.getClass().getSimpleName()));
		}
	...
	static boolean asBoolean(Object pValue) {
		if (pValue.getClass() != Boolean.class) {
			throw new JsonException(
					String.format("Attempting to retrieve an %s as a boolean", pValue.getClass().getSimpleName()));
		}
	...
	static JsonObject asJsonObject(Object pValue) {
		if (pValue.getClass() != JsonObject.class) {
			throw new JsonException(
					String.format("Attempting to retrieve an %s as a JsonObject", pValue.getClass().getSimpleName()));
		}
	...
	static JsonArray asJsonArray(Object pValue) {
		if (pValue.getClass() != JsonArray.class) {
			throw new JsonException(
					String.format("Attempting to retrieve an %s as a JsonArray", pValue.getClass().getSimpleName()));
		}
